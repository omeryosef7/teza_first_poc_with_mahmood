...
- [ ] **★ SCALE-UP (user chose 2026-07-20: stratified ~50):** built `scripts/build_stratified_perbehavior.py` → tags all 520 by domain (keyword heuristics), proportional seeded sample of **50 behaviors** (12 pilot reused + **38 new**), saved to `outputs/stage_gcg_percot_v2/STRATIFIED_SAMPLE.json`. Domains: malware 12 / other 9 / fraud 8 / weapons 8 / theft 5 / misinfo 4 / drugs 2 / self_harm 2. **38 new × 4 styles × 3 seeds = 456 opt runs**, 3 shards `joblist_scale_shard{0,1,2}.jsonl` (152 each). **⏹ STOPPED BY USER 2026-07-21: all SLURM jobs cancelled (`scancel -u`). Scale-up left PARTIAL at 105/456 opt runs (23%); 15/38 new behaviours touched, only 4 FULLY optimized (advbench_full_0114/0185/0276/0329, all malware_hacking), 11 partial. NO scale ASR computed (rerank never run). Killable priority had blocked new workers for many ticks (even idle GPUs → PENDING).** All state is on disk & resume-safe: to RESUME later, reseat workers `sbatch --export=ALL,JOBLIST=outputs/stage_gcg_percot_v2/joblist_scale_shard{k} slurm_scripts/run_gcg_percot_worker.slurm` (they skip completed runs); when the 38 are done → rerank harness (auto-discovers all 50). Or score the 4 fully-done now via `BEHAVIORS=advbench_full_0114,advbench_full_0185,advbench_full_0276,advbench_full_0329`. **TODO (paused):** drain via resume-safe workers `sbatch --export=ALL,JOBLIST=outputs/stage_gcg_percot_v2/joblist_scale_shard{k} slurm_scripts/run_gcg_percot_worker.slurm` (seat as L40S frees; probe-cancel if pending); when all 456 done → run rerank harness (`run_gcg_percot_rerank.slurm` or `_a5000` fallback, no BEHAVIORS = auto-discovers all 50) → **population ASR table by domain** → compare to pilot. NOTE: proportional sampling gives misinfo only 4 despite being the most-crackable domain (063/209) — population estimate is unbiased; a max-ASR run would oversample misinfo.
...
|---|---|---|---|---|
| 5A CoT-target (25 beh) | 10.7% | **4.0% (3/75) = baselines → zero uplift** | −6.7pp | 667598 DONE |
| Phase-8 λ0.3 (25 beh) | 24.0% | **10.7% (8/75), +8pp/neut** | −13.3pp | 667601 DONE |
...
**2026-07-19 ~21:46 — collapse is NOT universal (trending, 520 partials):** opt-vs-baseline uplift (current partials): **9A (refusal-dir s42) ~15% (25/163) vs ~2.5% → UPLIFT; 9B (CoT s45) ~6% (21/350) vs ~1.7% → UPLIFT; 7A/5A (CoT s42) ~3% vs ~2% → none; Gemma4-10A ~1.4% (7/488) < 2.3% → none.** So the 5A-gate "no uplift" is config/seed-specific, not a universal property — refusal-dir and seed-45 retain real uplift under correct placement. Directly motivates the per-behavior experiment (exploits this variance). Finalize per §13.6 when evals complete. (Benign SLURM `_cgroup_procs_check` msg in 667428.err = teardown noise, not a failure.)
...
| Phase-8 λ0.3 (25) | 24.0% | ✅667427 | ✅667601 **DONE** | **10.7% (8/75), +8pp/neut = real modest uplift** | complete (300/300); below v1 24% (prefill-inflated) |
...
- **07-20 per-behavior PILOT COMPLETE (12/12) + scale-up launched:** pilot finished (mean best-ASR 0.300, mean uplift +0.208, 9/12 crackable; full table above). Last 2 behaviours (209/229) scored on the **2×A5000 fallback** (job 669543, n-501) after L40S stayed fully booked — proved the A5000 path. User chose **stratified ~50** scale-up → built `build_stratified_perbehavior.py` (domain-tag 520 → proportional seeded sample; 12 pilot + 38 new = 456 opt runs, 3 shards). Diagnosed the standing blocker: **killable saturation is at the scheduler/PRIORITY level, not GPU count** — even fully-idle nodes reject jobs `PENDING(Priority)`. Once priority cleared, ran up to 3 workers; preemption + wall cycling dropped it back to 1, then 0.
- **07-21 CAMPAIGN STOPPED BY USER:** `scancel -u` all jobs. **Final state — pilot: COMPLETE 12/12 (documented). Scale-up: PARTIAL, 105/456 opts (4 of 38 new behaviours fully optimized, 11 partial), NO scale ASR computed.** Everything resume-safe on disk (see the scale-up TODO above for the resume recipe). Loop may still be scheduled (cron `35b00f3a`); stop with `/loop` off if fully done.
