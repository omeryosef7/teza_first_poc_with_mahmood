...
| `probe_boombness` in the §6.4 three-way metric comparison | **DONE** — `probes.py --emit-scores` adds out-of-fold per-prompt margins; comparison run. ⛔ **RETRACTED (#9)** — that comparison put probe (n=72) beside direction (n=270). Like-for-like on the common 72 with a demeaned within-domain permutation, **no metric predicts ASR once `n_examples` is partialled out** (direction +0.009 p=0.39; probe +0.095 p=0.65). All five required targets now present. |
