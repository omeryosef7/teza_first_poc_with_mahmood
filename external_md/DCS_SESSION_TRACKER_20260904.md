...
| 31 | `A-015` | ✅ The 5 new arms are **comparable**: same bank (`de4818a6c08fd6ad`), **0 `prompt_sha16` mismatches** on every shared row (611/611, 611/611, 515/515, 759/759, 431/431). ⛔ `prompt_id` alone is insufficient — `C-037c` kept ids identical across bank rebuilds. Now a **guard** in the primary, not a one-off | ✅ |
| 31 | — | ⚠ My first test of that guard **wasn't one**: I expected a Qwen arm to trip it, but `dcsqwb` uses the *same* `cds116` bank, so passing was correct — a different **model** is not a different **prompt corpus**. Re-tested with a synthetic arm carrying **40 corrupted** hashes: refuses by name and count, and still passes the matched K=3 set | ✅ |
| 32 | `B-016` | ✅ Judging **unblocked**, verified by a **real completion** (not just `/models` — `C-024` was a quota block, which returns 200 there and 429 only on a completion). ⛔ NEW LIMITATION: artifacts record the judge **alias**, never the served snapshot; the API returned **`gpt-4o-mini-2024-07-18`** but all `p24j` rows say only `openai/gpt-4o-mini`. ⇒ `PR-028c`'s drift cannot separate sampling noise from a **snapshot rotation**; caveat recorded, measurement still worth making | ✅ |
...
| 36 | `A-017` | ✅ `B-016` mitigation **fired live** (852324 logs `snapshot BEFORE: gpt-4o-mini-2024-07-18`, all 10 arms pre-flighted). ⛔ Session guard was blind to the failure mode `cpu-killable` most invites: a **preempted+resubmitted** run writes a 2nd `p28j_` dir per arm in a different session under an **identical prefix**. Now keyed on `RUNMETA.slurm_job_id` (verified: p24j all **851289**, live **852324**). ✅ Tested on the motivating case — refuses same-prefix/job-999999 which the old guard **would have passed** | ✅ |
| 37 | `R-070` | ⚠ Tested the primary's **independence assumption** before it runs (possible only because `R-066` persists draw positions). ✅ **0 identical draws** in 11,200 arm-pair-rows; overlap **25.54** vs **25.06** simulated-independent. ⛔ But excess is systematic: **+0.483 ± 0.097 = 5.0 SE**, ratio **1.0193**. ✅ Null correctly specified — reconstructed pool matches recorded `n_pool` **exactly, 300/300**. ⇒ Between-control sd **understates** population sd ⇒ primary is **mildly anti-conservative**; a null is unaffected, a positive carries the caveat. ⛔ Magnitude deliberately **not** converted to an ASR correction (`C-038`'s lesson) | ✅ |
| 38 | `C-042` | ✅ Judging healthy on the partial (`judge_status` **ok 1027/1027**). ⚠ PREVIEW (one incomplete arm, **not** a result): **134/1027 = 13.0%** of labels flip on byte-identical text but **net +2 ⇒ ~+0.0019** — almost exactly the 0.0020 `R-068` called the wrong low end. ⛔ **Not reversing `R-068`** on one partial arm (that is fleeing to the other endpoint within a day); magnitude is **open**, `PR-028c` decides it on 5x1160. ✅ `R-068`'s **design** conclusion is untouched — one-session judging is right because raw ASRs don't transport, not because the offset has any given size | ✅ |
| 39 | `R-071` | ⚠ INTERIM **1 of 5**: first complete re-judge arm ⇒ **NO SYSTEMATIC OFFSET**. 1160 byte-identical rows, **0** sha exclusions, **152 flips (13.1%)** but **78 up / 74 down, net +4** ⇒ drift **+0.00345**, binomial **p=0.808**, net **0.32 sd** from zero. ✅ **0 refusal flips** — `C-023` now backed by 1160 rows on top of `R-049`'s 380. ⛔ Not the preregistered result and **not yet** grounds to overturn `R-068`: this is the **baseline** arm at ASR 0.342, and the other four span `KO-3` 0.274 and controls 0.282-0.341 | ✅ |
