...
| The arm exceeds every in-subspace control at **L8/L10/L12**; ⛔ **at L6 the margin is inside judge-session noise (2026-08-22)** | `insubspace_null_by_layer.json` | ⛔ **L6 WITHDRAWN 2026-08-22.** Every delta here is `arm_ASR − baseline_ASR`, and the arm, the controls and the baseline were **judged in different sessions** — at L6 the 20 controls span **five** judging sessions, all differenced against a baseline judged in a **sixth**. Judging the *same byte-identical generations* repeatedly gives a session drift of **0.0057** (8 sessions; independently **0.0056** and **0.0061** on two other generation sets). L6's arm-vs-best-control margin is **0.0081 = 1.41× that drift**, so the comparison cannot be distinguished from which session a control happened to be judged in. L8/L10/L12 margins are **0.0242/0.0323/0.0263 = 4.2×/5.6×/4.6×** and survive. `judge_session_drift.json`. A concurrent line of work reaches the same L6 conclusion by a different route (20 controls, session-matched baselines, a control at 109.7% of the arm). Superseded text: ~~Ratio **1.80× / 2.33× / 3.60×** at L6 / L8 / L12; **at L10 it is undefined** because every control there is ≤0~~ — so "1.80×–3.60×" (an earlier revision) covered 3 of 4 layers, corrected 2026-08-22. ⬆ **Sweep COMPLETE at every layer 2026-08-23** — all four carry the **full 24 in-subspace controls**, `n_common: 495`, `population_matched: True` throughout (`insubspace_null_full24.json`); rank-p floor **0.04** (1/25). **And the null has CONVERGED: refining the grid did not raise the ceiling at any layer** — L6 12→24 angles, L8 12→24, L12 8→24, L10 **4→24** (a 6× increase), and the maximum control delta is **identical** in every case (5p / 9p / 5p / 0p). That matters because the standing worry about this null was that it is *a sample, not a bound* — that a denser grid would keep finding stronger controls. Between 12 and 24 controls per layer, it does not. ⚠ A concurrent line of work reports the opposite (L6 ceiling growing +0.0118 → +0.0172, "+46%"); that computation disagrees with this one at **both** endpoints and is not reproducible from any committed script or artifact, so the discrepancy is recorded, not resolved — this closes the gap a concurrent line of work flagged as unpaid (*"L10 still rests on 4 angles and L12 on 8"*). **No control reaches the arm at any of the four layers.** Margins in prompts against the measured judge drift of **2.8 prompts / 495**, now at completed resolution: L6 **4.0 (1.41×)**, L8 **12.0 (4.22×)**, L10 **16.0 (5.62×)**, L12 **13.0 (4.57×)**.0**, L8 **12.0**, L10 **16.0**, L12 **13.0**. ✅ **Session-mixing concern TESTED AND CLOSED (2026-08-22, `xL6_crossover.json`).** Because only one control session in the sweep has a baseline of its own, every control delta is cross-session, and I suspected the null's ceiling might be a judging artifact. **It is not.** All eight L6 angles were re-judged together in one new session with its own baseline: the gap between the ceiling-setting angles (k=11/13/15) and the rest moved **+4.6p → +4.5p**, and no angle shifted by more than **2 prompts**. Under a rule fixed before the numbers existed (<2p = artifact, >4p = real), the verdict is **REAL ANGLE EFFECT** — the ceiling is a property of the directions, and the cross-session construction of the null is **vindicated**, not undermined. ⚠ An earlier revision of this row proposed the per-session mean spread (L6 5.5p, L8 8.0p) as a stricter noise bound and concluded L8 cleared it by only 1.5×; **that bound is now known to be mostly real angle variation, not noise, and is withdrawn** — the honest scale is judge drift, **2.8 prompts**. L6's margin is genuinely **4.0 prompts = 1.41× drift**, so its withdrawal stands, but on the margin being small rather than on any artifact. Superseded: ~~Null strength is also **uneven**: 12 angles at L6/L8, 8 at L12, only **4 at L10**, whose rank-p can never fall below 0.20.~~ The earlier "~16 band-sds" scored against a 4096-d random band and is **withdrawn** on the same grounds as R-23 (that band is far too weak a null); ⚠ note R-23's registry row names the *~7* figure, so this withdrawal is recorded here and in §14-B, not as a separate ID. |
...
> rank-3 cell-mean span — the arm exceeds every control at **L8/L10/L12** (⛔ **not L6**: margin 1.41× judge-session drift), but by
