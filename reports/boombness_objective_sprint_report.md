...
verifier, but **six further audits (#5–#10) have since forced retractions R-23…R-27 and corrections C-11…C-14**. Read §0's *current state* block before anything else.
...
by **+0.2869** (142) — **binary ASR, pooled**, the same estimand as the +0.0424 quoted above. ⚠ **All four estimands, added 2026-08-22** because §18 quotes different ones: arm C is +0.2061 / +0.2024 / +0.1967 / +0.1895 and arm D +0.2869 / +0.2710 / +0.2722 / +0.2544 (binary-pooled / binary-clustered / continuous-pooled / continuous-clustered). The estimand discipline audit #8 forced on the small effect had not been applied to the large one. Both survive every multiplicity family tried (Holm ≤ 0.002) with inert
controls. **The `d_surface`-specific part is an order of magnitude smaller and sits on the
significance boundary** — ⚠ and that cross-channel comparison is **not dose-matched** — 38.2% vs 88.9% of generations changed
(`generation_change_arms.json`); see §0b.
...
| The arm exceeds every in-subspace control at **all four layers** (24 controls each); L6 is the weakest | `insubspace_null_full24.json`, `judge_session_drift.json`, `xL6_crossover.json` | **Ratios arm/ceiling: L6 1.80×, L8 2.33×, L12 3.60×; at L10 undefined** because every control there is ≤0. (An earlier revision quoted "1.80×–3.60×" as if it covered all four; it covers three.) **Sweep complete 2026-08-23** — all four layers carry the full **24** controls, `n_common: 495`, `population_matched: True`, rank-p floor **0.04**; **no control reaches the arm at any layer**. **The null has CONVERGED:** refining L6 12→24, L8 12→24, L12 8→24 and L10 **4→24** left the ceiling **identical** in every case (5p / 9p / 5p / 0p) — the empirical answer to the standing *"a sample, not a bound"* objection, though 24 directions in a 2-d complement is still a sample. ⚠ **L6's 2026-08-22 withdrawal is RETRACTED (audit #14): its stated basis was my own arithmetic error.** It rested on the margin being **1.41×** a judge drift of **0.0057**. That drift was wrong — computed across passes with different denominators, its maximum supplied by `ab_base_20260819_002240`, a run with **no `DONE.json`** whose 12 missing rows are exactly positions 483–494 (a contiguous tail — a killed job), **none of them malicious**, so truncation alone inflated that pass by +0.0016. I had kept it on the written claim that it was "a complete run missing 12 rows, NOT an in-flight one", which was false. On the **intersection** of ids across the complete passes the baseline is only ever 32/495 or 33/495: **drift = 0.0020, one prompt** *(re-derived 2026-08-23 on **13** such passes, up from 10 — unchanged, and no layer's verdict moves)*, giving **L6 4.00×**, L8 12.0×, L10 16.0×, L12 13.0× — **no layer is inside judge noise**. ⚠ **That reversal favours my own result, so its limits are flat:** it restores nothing beyond the drift comparison. L6 stays the weakest layer on every reproducible measure (**1.80×**, a **4-prompt** margin, both smallest), and the concurrent line's report of a control *exceeding* the L6 arm remains unreconciled and unreproducible from any committed script or artifact — as does its claim that the ceiling *grows* with grid density (+0.0118 → +0.0172), which disagrees with this computation at both endpoints. ✅ **Session-mixing concern tested and closed:** all eight L6 angles re-judged in one fresh session moved the ceiling-group gap **+4.6p → +4.5p**, no angle by more than 2 prompts — **REAL ANGLE EFFECT** under a rule fixed in advance, so the cross-session construction is vindicated. ⛔ An intermediate proposal to use the per-session mean spread (L6 5.5p, L8 8.0p) as a stricter noise bound is **withdrawn** — the crossover showed it is mostly real angle variation, not noise. ⛔ The earlier "~16 band-sds" scored against a 4096-d random band and is **withdrawn** on the same grounds as R-23 (that band is far too weak a null); ⚠ R-23's registry row names the *~7* figure, so this withdrawal is recorded here and in §14-B, not as a separate ID. |
...
to replace. ⚠ This is **not** significant at cluster level either (see "What is NOT established" in **§0a above** — that subsection moved during the 08-22 split); it is
...
**Read the magnitudes, not just the stars.** The refusal arm is **~5–7× larger** (0.2061/0.0424 = **4.9×**; 0.2869/0.0424 = **6.8×**; it reaches 11× only against L6's +0.0182 — ⚠ an earlier revision said "an order of magnitude", overstating by about 2×)
...
keyword detector (`judge_boombness.py:414`) — but it agrees with `score < 0.5` on **3432 of 3465 rows
(99.05%)** across the seven core arms — ⚠ an earlier revision said "3942 of 3960 (99.55%)", which
reproduces from no subset of the runs. Two instruments agreeing ~99% of the time *must* have near-equal-and-opposite deltas, so
...
so the judge-session term cancels rather than being carried as a judge-drift uncertainty (0.0020 — corrected 2026-08-23, audit #14). Judging session
...
3. **Any published p below its design's floor is not clustered evidence.** §2.6's 0.00099 (6 domains,
   floor 0.031) is a bootstrap/parametric number and must not be described as clustered. ⚠ **An
   earlier revision also cited §14-B's 0.0089 here — wrongly.** §14-B clusters on **16** domains
   (floor 3.1e-5), with 8 informative clusters at L8 (floor 0.0078), so **0.0089 is above both floors**
   and is *not* an instance of this rule. The example a reader would apply to the surviving headline
...
> rank-3 cell-mean span — the arm exceeds every control at **all four layers** (L6 weakest at 1.80×; ⚠ its earlier withdrawal on a "1.41× drift" basis is retracted — that drift was wrong, audit #14), but by
...
| id | what was asserted | status, and why |
|---|---|---|
...
| **R-25** | the **in-subspace null's interpretation** — "the arm beats every direction in the same rank-3 subspace, therefore the effect is about *this direction's content*" | **RETRACTED (interpretation only — every number reproduces digit-for-digit under independent recomputation).** The null was never **dose-matched**. `d_surface` is essentially **PC1** of the cell-mean span (cos with PC1 **0.9998–1.0000** at L6/8/10/12), so it removes **0.81–0.88** of that spread while every `in_subspace_angle` control — living in the orthogonal complement *by construction*, i.e. the two low-variance components — removes at most **0.13**: a **6–11×** dose gap unrelated to concept content. Within L6's 12-angle null, Spearman ρ(dose, Δ) = **0.961**; the smooth unimodal hump an earlier tick recorded as evidence the null was well-behaved *is the dose curve*. Dividing by dose is not a repair (the response saturates: the within-null OLS line extrapolates to +0.250 vs +0.018 observed), and a dose-matched in-subspace control **cannot exist** — the complement holds only ~0.16 of the spread in total. **Surviving:** `arm_exceeds_all_controls` is true at **L8/L10/L12** as measured; ⚠ **L6's 2026-08-22 withdrawal was RETRACTED on 2026-08-23 (audit #14)** — it rested on a judge-drift figure of 0.0057 that was computed across passes with different denominators and topped by a truncated run; the corrected drift is **0.0020**, making L6's margin **4.00×** rather than 1.41×. L6 is retained as the weakest of the four layers. **Withdrawn:** that this establishes content rather than magnitude. On a behaviour-level dose metric the arm **ties** its best control at L8 and L12; only **L10** survives every normalisation. Compounding it, the four layers are the **top four of eleven** ranked by the same statistic, with **no multiplicity correction**. `outputs/boombness/insubspace_null_by_layer.json` (`dose_confounded`, `DOSE_CAVEAT`, `LAYER_SELECTION_CAVEAT`). Found by audit #6. |
| **R-26** | **§14-D's specificity conclusion** — "the ASR effect is specific to `d_surface`" | ⚠ **NARROWED 2026-08-22 (audit #12), not a blanket withdrawal.** The retraction rested on **one** dose metric — fraction of the *fitting bank's* cell-mean spread removed, where `d_context` is 0.13 against `d_surface`'s 0.84. On the **evaluation-set** metric the report uses elsewhere, `d_context` changes **34.9%** of generations against `d_surface`'s **38.2%** — **91% of the perturbation rate** — and produces **zero** flips at cos **0.188**. That is a dose-matched inert control. **The two metrics disagree and this design cannot adjudicate between them**, so the honest position is: specificity is *supported* on the eval-set perturbation metric and *not* on the cell-mean-spread metric. ⛔ **The original blanket withdrawal was an over-retraction**, and it used `generation_change_arms.json` as a dose metric where it undercut a claim (§0b's cross-channel caveat) while refusing it where it would support one — the same asymmetric evidential standard R-13 and R-15 were retracted for, this time pointing at myself. |
...
#### ⛔ ★ Super-additivity — **"established against its own control" is WITHDRAWN (2026-08-22)**; the contrast against the control is **p=0.055**, and Holm over the session's tests gives 0.083
...
| **E5** | AdvBench super-additivity on 16 clusters | ⛔ **"ESTABLISHED" withdrawn (2026-08-22)** — +0.0268 [+0.0029, +0.0584] against a control **double** (not triple); the contrast is p=0.055; §7c |
...
See Q2. ⚠ **That claim was false when written (audit #17)** — Q5's `core2x2` pair (**−0.0832**, p **0.572**) was in **no** committed artifact, because `_CLEAN` is built with `--require-bank-block core2x2,extra_conditions,role_style,families` → n=90 and holds no core2x2-only subset. ✅ **Closed 2026-08-23** by running the analysis that was quoted but never executed: `g2_analysis_cwpos_CORE2X2.json` (`--require-bank-block core2x2`, n=**60**, 6 clusters) reproduces **both** figures exactly — `clustered_inference.rho_within_domain` = **−0.08319**, `p_perm_within_domain_rho` = **0.5722**. **All 13 answers are now fully sourced.** The lesson stands unchanged: the sourcing check tested only whether a **filename appears near an answer**, never whether the quoted figures are in that file, so its "0 flagged" was weaker assurance than it read — which is why `answer_regenerability_check.py` now checks each quoted figure against the artifacts the answer names.
...
✅ **The `core2x2` pair is now sourced (2026-08-23).** It had been quoted without ever being run: `_CLEAN` is built with `--require-bank-block core2x2,extra_conditions,role_style,families` → n=90 and holds no core2x2-only subset. Running `analyze_g2.py` with **`--require-bank-block core2x2`** alone gives `outputs/boombness/g2_analysis_cwpos_CORE2X2.json`, **n=60** over **6** clusters, and reproduces both quoted figures to the digit: `clustered_inference.rho_within_domain` = **−0.08319** and `p_perm_within_domain_rho` = **0.5722**. Note the artifact's *`layer_selection.headline`* carries a slightly different p (**0.5537**) from the maxT selection pass — the body quotes the **clustered_inference** value, and that is the one to cite. The bank filter is recoverable only from `provenance.argv` (there is no dedicated field), so cite the argv line, not a key. **The figures were right; nobody had checked they were reproducible, and for six days they were not.**
