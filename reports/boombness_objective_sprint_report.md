...
| #1 … #9 | the sprint's **`#N` retraction series** (a THIRD numbering) | **RECORDED — different series.** The body cites "retraction **#3**", "**#6**", "**#7**", "**#9**"; these live in `docs/BOOMBNESS_SPRINT_PROGRESS.md` and are **not** the R-N series. **#7 is the same event tabled here as R-12** (the fake control band). Listed so the registry is genuinely closed: **three** numbering series exist — `#N`, R-1…R-5 (progress log), and R-6…R-25 (this table). Do not open a fourth; take the next ID from this table. |
