...
| **G2** (§9) | Does Boombness predict attack success? | ⛔ **UNDER REVIEW (R-18).** The published ρ=+0.307 (n=234) is computed over a **heterogeneous** row set: `analyze_g2` filters on `condition`, **not** `bank_block`, so 72 rows are **sibling families sharing demonstrations** and 72 come from the three **experimentally-manipulated** designed-variance blocks. On the 90 rows that are neither, **ρ = +0.086** (per-domain mean +0.025 ± 0.126) — at the **0.4th percentile** of random 90-row subsets, so not a small-n artifact; the 144 dropped rows carry ρ=+0.403. **Not claimed false** — the clean subset is underpowered — but **not established** either. Re-analysis is CPU-only and pending. |
