...
| **Realised dose IS counted and IS persisted, and it scales with the head count** | `doublespeak_causality/pair_common.py:959` (`n_heads_edited`), `:995`, `:1006`, `:1011` (`n_edits += n_rows * n_heads_edited`); surfaced in `summary.json` as `knockout_liveness.median_prefill_edits` / `total_prefill_edits` | **Measured**: `csi1_basket_train_NEC_KO` (all 32 heads) records `median_prefill_edits 2052.0`, `total_prefill_edits 1385460`, `total_decode_edits 0`, `frac_rows_scope_live 1.0`, `scope_violations {}`. ~~A K-head arm must record **exactly K/32** of the all-head arm's prefill edits on the same rows.~~ **CORRECTED 2026-09-21 (S-176): this is the SAME false claim §4.2 carried, in a second place my first fix missed. The counter records MASK WRITES, not heads masked — the all-head arm never expands the head dim and records 1 (broadcasting to all 32), a K-head arm records K. So a K=8 arm records 8 × the all-head arm, i.e. 16 416 against 2052, not ¼ of it. See §4.2.** Dose matching between the candidate and its K-head controls IS still verifiable from the artifact — every K=8 arm has identical K hence identical counters — but the all-head-vs-K comparison is not a dose ratio and must not be used as one. |
...
| Per-head z capture / patch / ablate primitives | `pair_common.py:1066` `_attn_head_dims`, `:1074` `ZHeadPatch`, `:1118` `AllPositionZHeadAblate`, `:1166` `ZHeadCapture` *(line numbers re-verified 2026-09-21, S-176; the doc's original :1064/:1072/:1115/:1161 had drifted by 2–5 lines. The primitives themselves all exist and resolve by name.)* | `ZHeadCapture` retains grad on the o_proj input — this is the AtP primitive. `ZHeadPatch` is the true-patch validator. Unit-tested GPU-free: `doublespeak_causality/tests/test_zhead_synthetic.py`, `test_allposzheadablate_synthetic.py`, `test_hook_firing_synthetic.py:55-116`. |
