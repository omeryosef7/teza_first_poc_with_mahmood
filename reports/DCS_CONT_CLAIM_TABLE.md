...
| **A11** | 🆕 A ridge probe on the demonstration-side state predicts installation **within domain** | **CONFIRMED ON TEST** — it **cannot mediate A1** (`C-CONT-040`); its causal status at its own site is **untested** | 900 fit / 230 test slots | TRAIN +0.6241 → VALIDATION +0.6784 → **TEST +0.6054**, 23/23 domains, p=0.00050. Beats a size-matched demo-span control by **+0.3960** | `CONT-ENTRY 059`, `061`, `075` · `DR-072` |
