...
| **A13** | 🆕 §15's matched reference: cell C's demonstration-side state aligns with the **explicit-BOMB** state as installation rises, beating the matched context-only prototype | **MEASURED, REPLICATED CROSS-CODEWORD** | 900 slots / 90 domains each | button B−ctx **+0.4053**; basket **+0.3317**; both perm p=0.00050. **Concept-DEPENDENT, not bomb-specific**: knife/button gives **+0.0623** (p=0.00050 — real but **6.5× weaker**, against only a 1.63× variance deficit); gun inconclusive. ⚠️ installability and geometry strength are confounded across only 3 concepts | `CONT-ENTRY 077`–`079`, `082` |
