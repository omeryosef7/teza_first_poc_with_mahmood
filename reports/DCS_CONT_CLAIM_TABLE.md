...
| **A7d** | 🆕 **The codeword-dependence is not a vocabulary problem** | **TWO REPAIRS TESTED, BOTH REFUTED** | 40 labels | term-distribution shift: same shape on both codewords (`C-CONT-067`); require an energetic *substance*: **worse on both** (basket 0.73/0.62, button 0.40/0.40) even when fitted on its own labels (`C-CONT-068`) | `CONT-ENTRY 090`, `091` |
...
| any **button** content-true number | there is **no lower bound on button** — the conservative rule is precision 0.50 there (`CONT-ENTRY 090`) |
| "the correction can be fixed with a better word list" | **two repairs tested, both refuted** — the discriminator is structural, not lexical (`C-CONT-067/068`) |
...
| `C-CONT-062` | §15's matched pairing | does **no work** — mismatched +0.4059 > matched +0.3823; §46 prereq 7 → PARTIAL |
...
|---|---|---|
| **a content-true endpoint that works on button** | the binding constraint on the whole program. Term-based rules are **exhausted** (`C-CONT-067/068`): the discriminator is what the answer *does*, not which words it uses. Needs a function-reading judge validated **per codeword** against labels, or an accepted decision to report bounds | judge design + labelling |
...
| **Power on A2/A3 in content-true units** | with button ~72 % of the numerator spurious, the MDE is **103–132 % of the entire genuine attack rate** (`REVIEW-3/SCIENTIFIC`). Gated behind the endpoint | new rows + ~12 GPU-h |
...
profile** between them, and **two principled repairs both fail** — so this is not a tuning defect but a
property of term-based ASR judging on codeword-remapping attacks. 231 completions blind- or
