...
| **A4** | The same cut **cuts refusal to ~a third**, band-specific | **MEASURED, REPLICATED ON MATCHED HARDWARE IN ONE JUDGE MANIFEST** | 670 rows, 67 domains | published 0.115→0.040 vs 0.112 (cross-hardware); re-measured **ko 0.0403 vs ctrl 0.1119**, difference **−0.0716 [−0.0940, −0.0507]**, sign-flip p < 1e-9 (exact sign test) — the same two numbers to three decimals. **Negative in 33 of the 33 domains where either arm refuses at all** (0 positive; the other 34 domains are exact ties, 28 with zero refusals in both arms), so the effect has **no counterexamples** | `CONT-ENTRY 044`, `121` |
