...
| **A2** | The **same cut does not change attack success** | **NULL, REPLICATED CROSS-CODEWORD — but UNDERPOWERED in content-true units** (flagged by four consecutive reviews; corrected here). It is powered against the *lexicon* endpoint, not against genuine attack success: CR-002's button rate on the **ASR arms** is **0.0343** (the population this primary is computed on; 0.0250 is the 114-run corpus and 0.0263 its train+val subset — three populations, named here because the table previously quoted all three unlabelled) and the CI half-width ~0.029, so the MDE **exceeds the entire genuine attack rate** | 67 domains each | **button** +0.0030, CI [−0.030, +0.036]; **basket** −0.0090, CI [−0.0254, +0.0090], p=0.40. ⚠️ the basket **sign is not established** — under three content rules it is −0.0090 / −0.0030 / +0.0030, bracket **[−0.0090, +0.0030]** spanning zero (`C-CONT-063`, `CONT-ENTRY 087/089`) | `DR-070`; `DR-073a` |
