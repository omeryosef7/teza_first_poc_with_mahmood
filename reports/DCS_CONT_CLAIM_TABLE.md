...
| **A16** | 🆕 **Installation gates REFUSAL, not capability.** Across the dose ladder, installation, refusal and attack success all start at zero — and adding demonstrations moves refusal with installation while attack success stays near the floor | **MEASURED, `slot0` PRIMARY** | 180 slots/dose, 90 domains | dose 0 → 4: installation **0.0000 → 0.6587**, refusal **0.0000 → 0.1111**, content-true **0.0000 → 0.0389** (button). Per-domain, installation gain predicts refusal gain: **ρ = +0.2376, p = 0.023**, and the same in sign and size on the secondary (+0.2548) — the first scope-sensitive quantity in this phase that agrees both ways. The knockout moves the same two together downward (−0.2150 installation, −0.0716 refusal, **33 of 33** domains). 🆕 **`DR-075` SUPPORTED**: dose 4→8 raises installation only 10.3 % and refusal only **+0.0278** (vs +0.1111 for the first four demos) — refusal is **not** a demonstration counter. ⚠️ the CI **[−0.0111, +0.0667]** includes zero *and* exceeds the +0.0555 threshold, so this rules out a large rise without pinning a small one; the measured rise is **2.4×** the predicted +0.0115; and content-true rose proportionally **more** than refusal (+57 % vs +25 %), which the quantitative half of A16 does not predict | `CONT-ENTRY 129`, `121`, `100`, `136` · `DR-075` |
