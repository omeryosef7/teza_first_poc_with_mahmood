...
**BOMB representation search → causal ASR.** 2026-09-10 / 09-11. **Revised 2026-09-12 after REVIEW-2 … REVIEW-8** (`C-CONT-038`…`C-CONT-094`). ⛔ **A16 was entered and WITHDRAWN on 2026-09-12** (`C-CONT-092`).
...
declared controls that were not run before it. ⚠️ **`C-CONT-094`: TEST was ALSO read inadvertently** — `CONT-ENTRY 133`'s seven published numbers and `DR-075`'s frozen config inputs were computed with the 23 TEST domains in, undeclared at the time. Both are corrected in `CONT-ENTRY 139`; neither changed a verdict, but the earlier claim that *nothing else had touched TEST* was false and is retracted here. `button` and `basket` are never pooled. The independence unit
...
| **A2** | The **same cut does not change attack success** | **NULL, REPLICATED CROSS-CODEWORD — but UNDERPOWERED in content-true units** (flagged by four consecutive reviews; corrected here). It is powered against the *lexicon* endpoint, not against genuine attack success: CR-002's button rate on the **ASR arms** is **0.0343** (the population this primary is computed on; 0.0250 is the 114-run corpus and 0.0263 its train+val subset — three populations, named here because the table previously quoted all three unlabelled) and the CI half-width ~0.029, so the MDE **exceeds the entire genuine attack rate** | 67 domains each | **button** +0.0030, CI [−0.030, +0.036]; **basket** −0.0090, CI [−0.0254, +0.0090], p=0.40. 🆕 **hardware-matched button** (one GPU, one judge manifest): **−0.0075 [−0.0597, +0.0448]** on the `slot0` primary — null survives, sign still flips with scope (+0.0119 all-slots). ⚠️ the basket **sign is not established** — under three content rules it is −0.0090 / −0.0030 / +0.0030, bracket **[−0.0090, +0.0030]** spanning zero (`C-CONT-063`, `CONT-ENTRY 087/089`) | `DR-070`; `DR-073a` |
...
| **A3** | ⇒ semantic installation and behavioural attack success are **dissociable under intervention** — **qualitatively ONLY** | **MEASURED, EXPLORATORY, TWO-CODEWORD. The quantitative version is WITHDRAWN TWICE**: `C-CONT-083` (button, gap +0.0221→**−0.0044, P=0.59** on the declared `slot0` primary) and `DR-074` (basket, **+0.0116, P=0.29**, NOT SUPPORTED under a rule frozen in advance). Cause, **basket only** (`C-CONT-091`): the dose-0 arm holds **`slot0` only**, so any all-slots dose slope compares 1 slot against 5 — on basket that inflates the ASR dose slope **4.2×** (+0.0075 → +0.0313). ⚠️ **On button the same scope change moves installation 1.01× and ASR 0.89×**, so it CANNOT manufacture the gap: the button reversal's cause is **unexplained**, not established |  67 domains × 2 codewords | A1 ∧ A2 on **both** button and basket. ⚠️ same scope and bands; **"same dose" is NOT true across the pair** — the installation and ASR arms use different query templates and persisted dose totals 346,329 vs 1,385,316 (`REVIEW-2/SCIENTIFIC 1.3a`) | `CONT-ENTRY 049 §2`, `071` |
| **A4** | The same cut **cuts refusal to ~a third**, band-specific | **MEASURED, REPLICATED ON MATCHED HARDWARE IN ONE JUDGE MANIFEST** | 670 rows, 67 domains | published 0.115→0.040 vs 0.112 (cross-hardware); re-measured **ko 0.0403 vs ctrl 0.1119**, difference **−0.0716 [−0.0940, −0.0507]**, sign-flip **p = 0.0078** exact sign test **on the declared `slot0` PRIMARY** (8−/0+/59 tied) — the same two numbers to three decimals. ⚠️ `CONT-ENTRY 132` published **2.33e−10** for this, which is the **all-slots SECONDARY** scope quoted without saying so (`C-CONT-093`); on that scope the counts are 33−/0+/34 tied (28 structural zeros) and the effect has no counterexamples, but the **primary is the admissible number and it is eight orders of magnitude weaker** | `CONT-ENTRY 044`, `121` |
...
| **A12** | 🆕 **The doublespeak remap is strongly concept-dependent**: `bomb` installs far more readily than `knife` or `gun`, on both codewords | **MEASURED, DOMAIN-PAIRED** | 1080 shared slots / 90 domains | button: bomb−knife **+0.4314** [+0.386,+0.475] 89/90 domains; bomb−gun **+0.5166** 87/90. basket: **+0.3612** 88/90; **+0.3346** 88/90. All sign-flip p < 1e-9 ⚠️ **asserted, not run** — A12 was never passed through `dcs_cont_exact_tests.py` (`C-CONT-093`, `REVIEW-8/D3`); the bound holds by worst case but no test was executed. ⚠️ the `knife`/`gun` ordering **reverses** between codewords (+0.0852 vs −0.0267) and does not replicate | `CONT-ENTRY 079`, `080` · `C-CONT-057` |
| **A13** | 🆕 §15's matched reference: cell C's demonstration-side state aligns with the **explicit-BOMB** state as installation rises, beating the matched context-only prototype | **MEASURED, REPLICATED CROSS-CODEWORD** | 900 slots / 90 domains each | ⚠️ **contrast-dependent** (`C-CONT-059`): matched **single** reference **+0.1598** (button) / **+0.1703** (basket), p=0.0005 both; the *averaged* prototype gives +0.4053 but sits near zero by **cancellation**. Quoted conservatively. ⚠️ the token-level **matching does no work** — a *mismatched* same-domain partner scores **+0.4222 [+0.3879, +0.4510]** over 200 draws, beating the matched pairing in **98 %** of them and a domain-mean prototype **+0.5057** (`C-CONT-062`), so §46 prerequisite 7 is **PARTIAL**. **Concept-DEPENDENT, not bomb-specific**: knife/button **+0.0623 is NOT significant** against its own null (p=0.0845) and is **−0.0171** under the fair single-reference contrast; gun inconclusive. ⚠️ installability and geometry strength are confounded across only 3 concepts | `CONT-ENTRY 077`–`079`, `082` |
| **A14** | 🆕 **Installation is delivered almost entirely by the first four demonstrations**, and the knockout removes about a quarter of the total | **MEASURED** | 90 domains (ladder), 67 (share) | matched-`slot0`, probability scale: dose 0 ≈ 0 → **dose 4 +0.6728** [+0.610,+0.734] 90/90 → **dose 8 +0.0632** [+0.029,+0.099] 60/90, i.e. doubling the demos adds **9.4 %** of what the first four gave. Knockout share of the full 0→8 span: **25.4 % [20.8, 30.3]** — ~**three quarters survives the cut**. ✅ the 4→8 step's cross-run caveat is **discharged**: dose 4 re-measured on the dose-8 hardware gives an offset of **−0.0002 [−0.0025, +0.0020]**, and the step is **+0.0635 [+0.0296, +0.0963]** within-run | `CONT-ENTRY 093`, `094`, `100` |
| **A15** | 🆕 **The cut removes refusals and never adds them; what replaces them is mostly not an attack** | **MEASURED, STRUCTURE REPLICATED ON TWO CODEWORDS** | 1340 paired prompts, matched hardware, one judge manifest each | **STRUCTURE (both scopes): `ko`'s refusal set is a strict SUBSET of `ctrl`'s** — 27 ⊂ 75 (button), 1 ⊂ 8 (basket); **0 reversals** in 1340 paired prompts, and 0 on the `slot0` primary too. ⚠️ **QUANTITIES ARE ALL-SLOTS ONLY**, the scope `CONT-ENTRY 120` declared *secondary* (`C-CONT-089`): de-refused 48/7, leak **10.4 %**, **domain-clustered CI [0.024, 0.192]** over 33 domains (hand-labelled **12.5 %**, 0 false positives). ✅ the row-level Wilson interval previously quoted, [0.045, 0.222], was on the **wrong unit** (flagged by REVIEW-7 and REVIEW-8); recomputed on the declared DOMAIN unit it is **0.95× as wide** and the conclusion is unchanged — de-refusals are spread, at most 4 of 48 in any one domain. ✅ **TEST-clean**: the de-refusal set is byte-identical on train+val and all-domains, contribution +0.0075/+0.0015. On the declared **primary** there are **9 and 1** de-refusals and **0** content-true on button — too few events to analyse. ⚠️ one-directionality is **near-definitional** under the `kw_refusal` substring detector and **goes bidirectional under the judge's own rubric refusal** | `CONT-ENTRY 121`–`124` |
| **A16** | ~~Installation gates REFUSAL, not capability~~ | ⛔ **WITHDRAWN** (`C-CONT-092`, `CONT-ENTRY 138`, `REVIEW-8`) — entered `CONT-ENTRY 130`, withdrawn the same day. Three independent defects: (1) **no dissociation exists** — at dose 4 refusal 0.1111 is **0.99×** the control arm's ordinary rate and content-true 0.0389 is **1.13–1.56× ABOVE** the corpus rate, so both endpoints sit at their ordinary full-attack levels and 0.111-vs-0.039 is a base-rate ratio read as a difference in kind; (2) **the ρ is not a dose-response test** — dose-0 refusal is 0 in **90 of 90** domains, so "gain" is algebraically "dose-4 level" and ρ=+0.2376 is the cross-sectional correlation *inside* the dose-4 arm; (3) **the event counts are 20 refusal and 7 content-true rows**. Also: ρ=0.2376 is **below this design's own MDE** (≈0.29 at n=90) and `CONT-ENTRY 124`'s ρ=0.176 CI **contains** it while being called not-supported; the "+0.2548 secondary agrees both ways" is an all-slots dose contrast `CONT-ENTRY 134` rules **inadmissible**; and the headline ρ had **no committed script or artifact** | — | withdrawn; no number from this row may be quoted | `CONT-ENTRY 130`, `136`, **`138`** · `REVIEW-8` |
| **A17** | 🆕 **The simplest rival account is excluded on both codewords.** A *one-switch* model (installation gates whether the request is read as a bomb request; refusal and bomb content both downstream) requires the knockout to cut both endpoints by a similar proportion. It does not | **MEASURED, SECONDARY SCOPE, REPLICATED CROSS-CODEWORD** | 670 prompt pairs / 67 domains, each codeword | content-true ratio ko/ctrl **1.333 [0.838, 2.250]** (button, 24→32 events) and **1.357 [0.750, 2.600]** (basket, 14→19) against refusal ratios **0.360** and **0.125** — one-switch **EXCLUDED** on both; GATING (ratio ≈1.0) not excluded. ⚠️ **all-slots SECONDARY only**: the declared `slot0` primary has **7→6** (button) and **1→2** (basket) content-true events and discriminates nothing (intervals [0.25, 3.0] and [0.0, 5.0]). ⚠️ only the **proportional** form is excluded — a loosely-coupled variant predicting 0.36–1.0 survives. ⚠️ basket's refusal arm is **8→1 events**, so the ratio it is compared against is barely measured. ✅ **survives the mechanical confound** (`CONT-ENTRY 145`): a refused row can carry no bomb content, so the less-refusing arm has more opportunity — decomposed, raw 1.333 = **opportunity 1.081 × conditional 1.234** (button) and 1.357 = **1.011 × 1.343** (basket). Conditioning on not-refused, the ratios are **1.234 [0.776, 2.073]** and **1.343 [0.741, 2.577]** and one-switch stays **EXCLUDED on both**. ⚠️ both conditional CIs include **1.0**, so no *rise* is established — only that content-true does not fall proportionally. ✅ **the comparator is verified INERT** (`CONT-ENTRY 146`): on basket, where all three arms sit on one GPU, the band control moves neither endpoint against an unintervened base — refusal **8 vs 8 (ratio 1.000)**, content-true **14 vs 15 (0.933)** — so ko/ctrl ≡ ko/base (refusal 0.125, content 1.267). ⚠️ this check is **basket-only**: button's `base` arm ran on a **V100** while its ko/ctrl ran on an **A5000**, and cross-GPU generation differs in 573/670 completions | `CONT-ENTRY 143`, `144` · `REVIEW-8` |
| **A8** | The transplant instrument **can** transfer, token-matched | **MEASURED** | 16 domains | +10.7 %, CI [+3.5, +19.5], p=0.010 | `CONT-ENTRY 021`, `023` |
| **A9** | 🆕 **Raw LLM-judge ASR overstates content-true attack success by close to an ORDER OF MAGNITUDE on codeword-remapping attacks** | **MEASURED; correction validated out of sample and then CENSUSED** | 60,478 rows / **114 runs** (census recomputed, not cached); 129 keeps labelled | `button` raw 0.2984 → lexicon → **CR-002 0.0250**: factor **11.9×, domain-clustered CI [9.6×, 15.6×]**; **≈8.1×** after *design-weighted* recall (0.706, not the naive 0.923). `basket` **6.6× [5.1×, 9.5×]**. ⚠️ threshold-conditional: **7.80× [6.33, 10.04]** at SR ≥ 0.75 and **12.74×** at SR ≥ 0.25, both on the full corpus (an earlier **6.0×** in this table was a six-arm figure and is corrected); `carrot` unvalidated | `CONT-ENTRY 099`, `105`, `106` · `scripts/dcs_cont_asr_factor.py` |
...
| `C-CONT-059` | §15 p-values were the null of a **different quantity** than the column they sat in |
| `C-CONT-071` | the published labels file had a **shuffled column**; 13 of 40 labels misaligned |
| `C-CONT-093/094` | a headline exact p on the **secondary** scope unlabelled (8 orders of magnitude), and TEST inside two published populations — one day after writing the guards for each |
| `C-CONT-092` | **A16**: a dissociation asserted from two endpoints that are both at their ordinary rates; a "dose-response" ρ that is algebraically cross-sectional; 20 and 7 events; a secondary this phase's own helper rules inadmissible; and no committed script for the headline number |
| `C-CONT-072/083` | a ratio built from **two different slot scopes**, neither stated — and repeated three entries later inside a bigger claim |
| `C-CONT-074/081` | "corrected" one **random draw** with another; the comparator had no interval |
| `C-CONT-077` | a **naive recall** used to adjust a headline, from a sample that oversamples by design |
| `C-CONT-079` | "full text read" was false of **100 rows out of 100** |
| `C-CONT-082/084` | asserted a run/arm **did not exist** without opening the directory — **twice in three entries** |
| `C-CONT-075` | let the scheduler pick the hardware for a job whose purpose was to **fix a hardware confound** |
...
| **Power on A2/A3 in content-true units** | **worse now that the endpoint exists**: against CR-002's button base rate **on the ASR arms** of **0.0343**, the primary's CI half-width (~0.029) is ~85 % of the entire genuine attack rate. The endpoint is defensible; it is not powerful | new rows + ~12 GPU-h |
...
| **A15** 🆕 in section A | — see the new row there; the three items below share one root, specified in `reports/DCS_CONT_NEXT_BANK_SPECIFICATION.md` | — |
...
> **7.80× [6.33, 10.04]** at a stricter SR ≥ 0.75 threshold. Basket **6.6× [5.1×, 9.5×]**.
