...
| **A15** | 🆕 **The cut removes refusals and never adds them; what replaces them is mostly not an attack** | **MEASURED, STRUCTURE REPLICATED ON TWO CODEWORDS** | 1340 paired prompts, matched hardware, one judge manifest each | de-refused **48** (button) / **7** (basket); **`ko` refuses where `ctrl` did not: 0 and 0**. Of button's 48: 54.2 % no concept vocabulary, 14.6 % literal build, **10.4 % [4.5, 22.2] content-true**. Contributes **+0.0075** to `ko`'s CR-002 rate against a null `ko − ctrl` — so A2's null is "the cut does a different thing than the endpoint measures", not "the cut does nothing". ⚠️ the *rate* does not replicate (basket 2/7, CI [0.08, 0.64]); and the installation→refusal link is **ρ 0.176 [−0.05, +0.40], MDE 0.34** — not supported, and underpowered | `CONT-ENTRY 121`–`124` |
