...
| **A12** | 🆕 **The doublespeak remap is strongly concept-dependent**: `bomb` installs far more readily than `knife` or `gun`, on both codewords | **MEASURED, DOMAIN-PAIRED** | 1080 shared slots / 90 domains | button: bomb−knife **+0.4314** [+0.386,+0.475] 89/90 domains; bomb−gun **+0.5166** 87/90. basket: **+0.3612** 88/90; **+0.3346** 88/90. All sign-flip p < 1e-9. ✅ **now RE-DERIVED and run exactly** (`CONT-ENTRY 157`, `scripts/dcs_cont_a12_exact.py`): domain-level sign counts recomputed from the raw one-word/cellC channel reproduce the asserted counts exactly (button 89/1, 87/3; basket 88/2, 88/2 of 90, train+val, TEST excluded) and the exact two-sided sign test gives **p = 1.5e−25 / 2.0e−22 / 6.6e−24 / 6.6e−24** — closes `C-CONT-093`/`REVIEW-8/D3` (trap #8). ⚠️ the `knife`/`gun` ordering **reverses** between codewords (+0.0852 vs −0.0267) and does not replicate | `CONT-ENTRY 079`, `080`, `157` · `C-CONT-057` |
...
| **A17** | ~~The one-switch rival is excluded on both codewords~~ | ⛔ **WITHDRAWN** (`C-CONT-096`, `CONT-ENTRY 147`, `REVIEW-9`) — entered and withdrawn within a day. **The entire excess was mechanical.** Two channels, not one: (a) refused rows cannot carry content (`CONT-ENTRY 145`), and (b) **CR-002's `SCOPE` clause is a disclaimer detector and the knockout suppresses disclaimers** (already on record as A2b), vetoing **4 control vs 1 knockout** candidates on button and **3 vs 0** on basket. Strip both — never-refused-in-both stratum, no disclaimer veto — and the ratios are **28/28 = 1.000 [0.630, 1.625]** and **18/17 = 1.059 [0.583, 1.900]**. Also: **the formalisation was wrong** — a single graded latent with two ordered thresholds *is* one switch, is tightly coupled, and predicts refusal-down/content-flat, so ratio≈refusal-ratio was never a theorem. And the conditional adjustment in `CONT-ENTRY 145` **conditioned on a post-treatment variable** | — | withdrawn; no number from this row may be quoted | `CONT-ENTRY 143`–`146`, **`147`** · `REVIEW-9` |
...
| `C-CONT-096` | **A17**: an exclusion built on a ratio whose entire excess was mechanical — a disclaimer-detector veto that the intervention itself suppresses — plus a formalisation that never captured the rival, plus an adjustment conditioned on a post-treatment variable |
