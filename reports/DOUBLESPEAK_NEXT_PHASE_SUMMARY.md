...
| `R-041`/`R-043` ⚠ **narrowed twice — `A-009`/`R-051`, then `R-053`** | **Fully-installed domains lose MORE under `KO-3` than partially-installed ones.** ⚠ **Categorical, NOT a continuous gradient.** ρ<sub>KO</sub> is negative everywhere measured — **−0.594 / −0.444 / −0.734** full-sample, **−0.352 / −0.601** within the varying subranges — and robust to leave-one-out and to three operationalisations. ⛔ **But the knockout-SPECIFIC part (contrast vs a dose-matched control) is demonstrated only ACROSS the full installation range.** Within the varying subrange: Llama **−0.503, p = 0.343** (n=13) and Qwen **−0.173, p = 0.504** (**n=30**, so power is NOT the explanation). ⚠ There ρ<sub>ctrl</sub> = **−0.428** nearly matches ρ<sub>KO</sub> = −0.601 — what **regression to the mean** predicts (`R-040`) | 2 models × 38 domains; claim audited twice (`A-009`, `PR-022`) through the same committed code path | ⚠ **CORRELATIONAL and CATEGORICAL**; dominated by **ceiling-vs-rest** on **both** models |
...
* ⛔ **"attack `C` only failed for lack of power"** — `PR-022` ran it at **n = 30** on Qwen and the contrast was
  **smaller** (−0.173, p = 0.504), not merely noisier.
