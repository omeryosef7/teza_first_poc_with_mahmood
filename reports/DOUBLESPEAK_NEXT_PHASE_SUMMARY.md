...
| `R-024` | **`KO-3` REPLICATES ON QWEN3-14B in SIGN and DOMAIN SPLIT.** ⛔ **NOT at "~3× the magnitude"** — that comparison is invalid and `R-023` said so *before* it was made (Qwen's baseline is ~2× Llama's over a **more bimodal** distribution, `frac>0` 0.813 vs 0.942, so a mean-only cross-model ratio misleads); the doses are also unmatched (Qwen **91,872** mask cells over 11 layers vs Llama **66,816** over 9) on a quantity `R-022` showed is steeply dose-graded. Cell `C` baseline +10.140 → `KO-3` **−13.080** (frac > 0 collapses **0.813 → 0.021**), control inert at +10.357. **−23.437 vs control, 1+/37− domains, p = 2.838e-10** — the *identical* domain split as Llama. Band 7–17 (same relative depth as 6–14 on 40 layers), `--enable-thinking false`, capability gate passed first (`R-023`) | **6** arms × 380 × 38 domains | **CROSS-MODEL (direction only)** |
...
| `R-030`/`R-031` | **The DESTRUCTIVE effect lives in layers 0–14 and peaks at 10–14; above 14 it is absent but the region is NOT inert.** ⛔ *"absent above 14"* overstates: 24–31 gives **+0.754 on 38+/0− domains, Holm p = 2.9e-11** — the most consistent sign pattern in the whole sweep, and unexplained. ⚠ The above-14 bands are 9L/8L and **not dose-comparable** to the equal-dose 5L bands; `R-037` further shows this inertness is **bank-specific**. Equal-dose 5-layer bands: 0–4 **−3.39**, 5–9 **−2.99**, **10–14 −5.65** (0+/38−) — **no band null**; coarse sweep adds 15–23 **+0.15**, 24–31 **+0.75** | 14 arms × 380 × 38 domains | **GRADED, not bounded at 6–14** ⚠ the inherited window contains the peak but layers 0–5 also contribute |
...
3. **Does `R-010` hold on a second concept?** Not run. ✅ **On Qwen3 it does** (`R-024`) — the
   mechanism replicates cross-family in sign and domain split (⛔ **not** at a claimable magnitude ratio — `R-023` pre-declared that comparison invalid), which makes the standing dissociation
