...
beats every direction in the same rank-3 cell-mean subspace at **L8/L10/L12** (2.33× / 3.60× at L8/L12; ⛔ **L6's 1.80× is withdrawn** — ⚠ its 1.41×-drift basis was retracted by audit #14 (the drift was wrong); L6 is retained as the weakest layer, arm/ceiling 1.80×); ⚠ this comparison is **dose-confounded** — R-25 **retracted** the reading that it shows *content* rather than *magnitude*;
...
does not. The **retracted** §18 "capability channel" (R-22) is still closed from the behavioural side,
on the separate observation that `d_surface` produces no gain on already-non-refusing prompts. One asymmetry worth keeping: `d_surface` converts
