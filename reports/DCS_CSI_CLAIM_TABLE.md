...
| # | claim | statistic (with CI and n_domains) | population / split / codeword / layer | caveats | source |
|---|---|---|---|---|---|
| D31 | **The codeword dissociation's behavioural channel is NOT confounded with extraction hardware. Basket's fitted direction is the SAME direction whether its activations came from a Tesla V100 (emulated bf16) or an RTX 3090 (native).** REVIEW R11's MAJOR-1 is closed by measurement | **abs(cos(w_new, w_committed))** on `cand_rank1`: **L18 = 0.998406**, **L20 = 0.997866**. Residual 1 − abs(cos) = **0.001594** and **0.002134**. *(CORRECTED by S-173 / REVIEW R13: these were originally called "float noise inside bf16's ~2⁻⁸ = 0.0039". That is wrong twice — 1 − cos is SECOND order in the angle (θ²/2 reproduces both exactly), so it was compared against a FIRST-order precision; and the activations themselves differ by median **2.57 % / 2.76 %** relative, ~7× outside that bound. **The corrected statement is STRONGER: inputs differing by ~2.6 % still yield directions agreeing at cos 0.998 — robustness, not noise.)* **Calibration scale, from D27's own committed numbers**: 46 controls against the native axis give min 0.0001 / median 0.0156 / **max 0.1894**, and the cross-codeword swap axis is 0.5569. All four vectors unit-norm, dim 4096. **Fit population identical on all four**: 670 fit rows / 67 TRAIN domains. **The A/B is one token**: `--layers 0,2,…,31` → `--layers 18,20`, with **33 non-exempt arguments verified identical**, bank sha16 `79511d9e254571e6` equal to the committed axis's own record, model revision `0e9e39f249a1` on **both** runs, and corpus population `3720 → 3714` on both | **670 fit rows / 67 TRAIN domains**, basket, **L18 and L20**, site `rel-6` (resolved BY NAME), `--fit-prompt behavioral`, seed 20260915. New corpus `cont1re_behavioral_basket_bomb_20260921_081723_897277`, job **914750**, RTX 3090 `compute_capability 8.6` read from RUNMETA and not the sbatch line. Gate 0 PASS on VOID **1, 2, 3 and 7** (incl. VOID 7, both committed axes byte-unchanged). *(CORRECTED by S-173 / REVIEW R13: this row and the gate's own banner said "all 7". VOID 5 (refit --readout/--site/--seed/--codeword) and VOID 6 (no threshold edited after a cosine exists) are NOT checked by any line of that gate.)* | **The threshold and the prediction were committed at 08:07:11, nine minutes before the job existed** — git history, not a sentence in a file. **This closes ONE candidate explanation and nothing else**: it is NOT evidence for any other, it says NOTHING about button (no button arm exists here; pooling forbidden by §3.3), and it does NOT mean basket's axis is *correct* — a direction unchanged under a hardware swap is not thereby a right direction. **The argmax over the COMMITTED nine-layer grid was not re-run** — the new corpus carries `layer_grid [18, 20]`. *(CORRECTED by S-172: a TWO-layer argmax WAS computed and it REVERSED — committed L18 0.6525 > L20 0.6512, new L18 0.6540 < L20 0.6545, both new axes recording `layer_argmax_not_used: 20`. The hardware moved the margin by 0.0018 where the margin that selected L18 was 0.0013, so **the selection is less stable than the direction it selects between**. At 0.0005 with no CI the two layers are tied at this precision, which supports D2/D13's plateau caveat rather than undermining it.)* **A code-identity difference was found and checked**: the new corpus captures **23 sites** against the committed 20 (three extra `cw_demo_*` sites appended after `cw_demo_mean`) with all 33 args identical, i.e. the extraction code changed — inert for the fit, because `dcs_csi_axis.py:67` resolves the site by NAME and `:68` the layer by VALUE, measured rather than assumed | **S-167**, **S-168**, **S-169** (`reports/DCS_CSI_PR009_HARDWARE_AB.json` 2 457 B, prereg `configs/dcs_csi_pr009_basket_reextract_3090.json`, axes `configs/dcs_csi_axis_basket_behavioral_RE3090_L18.pt` / `..._L20.pt`) |
...
| AM-22 | **D21's caveat** | *"Also **TRAIN-only on the button-L18 side** — no button-L18 VALIDATION arm has ever been run"* — **WITHDRAWN.** 54 button-L18 VALIDATION arms were run and read on 2026-09-21 (D29). D21's statistics and its reweighting caveat are **unaffected** | The 2026-09-21 sweep amended this identical clause in four sibling rows (AM-16 D25, AM-17 D23, AM-18 D24, AM-19 D13) and **skipped D21, which carries it in the same words**. Sections A8/A9/A10 also carry no *"Checked: D1–Dn … rows not listed are UNAFFECTED"* line, unlike the three earlier reconciliation blocks, so there was no declaration that D21 had even been inspected | **S-173**, REVIEW **R13** |
| AM-23 | **The two audit rows at ~316 and ~599** | Both quote D13's first caveat (*"TRAIN only — button has no held-out L18 arms"*) and assert it *"still stands and is not discharged"*. **That assertion is superseded by AM-19.** The audit rows' own headers read *"Verdicts, with the entry that now governs each"*, so they present as currently-governing rather than as dated snapshots, and neither carries the in-place `[SUPERSEDED]` marker this file uses elsewhere (line ~499) | Re-running S-163's sweep on the pre-reconciliation file returns **five** hits (30, 316, 498, 510, 596); the S-163 table recorded **three** (30, 498, 510). Lines 316 and 596 were in the grep output and were dropped, and S-165's sweep was scoped to headlines so it could not have caught them either | **S-173**, REVIEW **R13** |
| AM-24 | **`configs/dcs_csi_pr006_axis_swap.json:75` and D13's fairness basis** | Both rest an argument on basket's `selected_layer = 18` **being** its argmax (*"L18 = 0.6525 vs L20 = 0.6512"*) while button was FORCED to L18 against its own argmax. **On native bf16 basket argmaxes to L20 as well**, so that asymmetry — and the "the swap is clean" argument built on it — is **hardware-contingent**. Not withdrawn: at a 0.0005 margin the layers are tied, which is D2's plateau, and D13/D14/D15 ran both diagonals | Surfaced by REVIEW R13's verifier as a consumer of the flipped ordering that S-172 did not name | **S-173**, REVIEW **R13** |

---

## A11. NEW ROW — PR-CSI-010, the head-level causal experiment for `basket`, read once on the held-out split (2026-09-22)

| # | claim | evidence | population / scope | what it does NOT say | source |
|---|---|---|---|---|---|
| D32 | **PR-CSI-010: eight attention-head INDICES, applied across blocks 6-14 on the query-codeword row, carry most of the A1 knockout's effect on semantic installation for `basket` — and it REPLICATES on the held-out split at the preregistration's own bar.** The verdict is **WE FOUND (part of) THE WRITER**, which the design predicted in advance it would *not* see | **VALIDATION, 23 held-out domains** (job **916536**, 24/24 arms, one model load): `E(HD_KO)` = **−0.226961** ci95 **[−0.275864, −0.178680]** (GATE 1 PASS, upper bound < 0); `E(HD_TOPK)` = **−0.195984** ci95 **[−0.243068, −0.153491]**; `E(HD_BOTK)` = **−0.012567**; 20 controls **mean −0.004006, median −0.003160, sd 0.013205, range [−0.024571, +0.019694]**, **0 of 20 more negative than the candidate**; **rank 1 of 21**, `p` = **0.047619** == the attainable floor **exactly**; **F = 0.8635** ci95 **[0.7871, 0.9478]** — excludes 0 and ≥ 0.50. Candidate **14.5 sd** below the control mean and **3.87×** the controls' whole spread from the nearest. **TRAIN (descriptive) replicates it**: −0.234060 / −0.213094 / −0.005609, F **0.9104**. **INDEPENDENT RE-DERIVATION sharing no code** (`dcs_csi_rederive_subspace.py --direction necessity`): `candidate_minus_base` **−0.19598** against W4's −0.195984, **\|Δ\| = 3.5e-06**, rank **1 of 21**, `instrument_capable true`, verdict **PASSES**. **The uniform null is refuted**: §6 predicted ≈ **−0.057** for any 8 heads; 20 held-out draws give **−0.004 ± 0.013** | **230 rows / 23 VALIDATION domains** and 670 / 67 TRAIN, `basket`, band **6-14**, scope `target_surface_row_only`, `semantic_one_word`, cell **C**, dose 4, **K = 8**, 20 controls drawn `K`-of-(32−`K`) from the non-candidate pool. **GATE 0 passes on all 48 arms**: `HD_BASE` prefill **0**, `decode 0` and `violations 0` everywhere, `hooks_after 0` after every arm. **Realised-dose identity EXACT and relative**: train 16416.0/2052.0 and validation 16128.0/2016.0 both = **8.000000** = `K`. All arms one allocation per split on **RTX 3090**; identical `prompt_id` sets across every arm on **both** splits; **no TEST domain**; `slurm_job_id` uniform per split | **It is 8 head INDICES, not 8 (layer, head) CELLS.** `--knockout-heads` ties an index across 6-14, so **nothing here resolves which layer a head acts at** (§10) — even though the screen's largest single cell was **L14 h19** at −398.87. **Leg (i) only**: leg (ii) (`rel-11 → rel-6`) has no scope and is untested. **Blocks 0-5 and 15-18 are untouched.** **No cross-codeword transfer**: nothing here concerns `button`, and pooling is forbidden (§3.3). **The TRAIN rank is selection-contaminated by construction** and adjudicates nothing — R17 recorded that the train picture is the identical signature of a real localisation and of a selection artifact, and only the held-out split separates them. **The design's own honest expectation FAILED**: §6, before any data, expected *PARTIALLY LOCALISED or DISTRIBUTED with F well under 0.50* and said a positive verdict *"should read as the surprise it would be"*. It failed **toward a stronger result**, the direction most likely to indicate a mistake; what stands against that is that nothing was chosen after the fact — `K` fixed before any AtP number existed, head sets and 20 seeds frozen before any arm ran, the read frozen before the arms existed and amended only while **no endpoint value had been read** (S-210), and two independent analysers agreeing to six decimals. **`F` is a ratio of domain means, not an additivity claim**: knockouts do not compose, so `F = 0.86` does **not** license "the other 24 heads contribute 14%" | **S-213** (TRAIN, descriptive), **REVIEW R17** (the selection-contamination analysis), **S-218** (VALIDATION, adjudicating) — `reports/DCS_CSI_PR010_HEAD_validation.json` 18 959 B, `reports/DCS_CSI_PR010_HEAD_validation_REDERIVE.json` 4 625 B, `reports/DCS_CSI_PR010_HEAD_train.json` 19 027 B, prereg `configs/dcs_csi_pr010_head_causal_basket.json`, frozen read `runargs/dcs_csi_pr010_read.txt` |
