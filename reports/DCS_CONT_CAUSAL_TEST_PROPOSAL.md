...
The Phase-4 candidate sweep is complete (CONT-ENTRY 154): F5 (demo-side rank-1 ridge) is the sole
family that predicts installation and transfers, and it is **causally inert** — its site
`cw_demo_mean` is bit-identical across `ko`/`ctrl` (C-CONT-040), so it cannot mediate the one
intervention this phase owns. The query-side probe (F8, CONT-ENTRY 152) is the only candidate whose
...
(`knockout_applied=False`, `arm=None`), captured on bank `dcd92d723f3e6d00`, dose 4, layers
[0,2,…,31], sites incl. `cw_query`(==rel-11), `rel-6`, `cw_demo_mean`. So only the **`ko` arm** must
be generated, matched to that bank so prompt_ids pair exactly.
...
- **Negative close** iff: the query-side state differs across arms but the readout shift is NOT
  ...
  causal *mediator* — a clean, publishable conclusion.
