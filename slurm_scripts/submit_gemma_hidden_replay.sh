...
#SBATCH --job-name=ae_replay_gemma4
#SBATCH --output=logs/ae_replay_gemma4_%A_%a.out
#SBATCH --error=logs/ae_replay_gemma4_%A_%a.err
#SBATCH --ntasks=1
...
# Stage AE — Gemma4-E4B-it hidden-state replay. Arrays over completed generation
# shard files under $RUN_DIR/generation/gemma4/shards/*.jsonl (sorted). 4h wall
# time is a conservative starting point, to be tuned after the smoke test.
#
# Based directly on slurm_scripts/stage4_8_cond_g_goals4_10_gemma4.slurm.
#
# Requires RUN_DIR passed via --export, e.g.:
#   sbatch --export=ALL,RUN_DIR="$RUN_DIR" slurm_scripts/submit_gemma_hidden_replay.sh
...
mkdir -p logs "$RUN_DIR/gemma/hidden_states" \
...
echo "=== Stage AE Hidden-State Replay — Gemma4 Array Task ==="
date
hostname
