...
#SBATCH --job-name=ae_gen_qwen3
#SBATCH --output=logs/ae_gen_qwen3_%A_%a.out
#SBATCH --error=logs/ae_gen_qwen3_%A_%a.err
#SBATCH --ntasks=1
...
# Stage AE — Qwen3-14B generation (conditions A/D/E/G x goal_index 0-10).
# Array indexing: task_id = goal_index*4 + cond_idx, cond_idx in {0:A, 1:D, 2:E, 3:G}.
# 44 tasks total, throttled to 3 concurrent (combined with Gemma's %3 throttle,
# keeps total in-flight SLURM jobs at <=6 per user's hard parallelism rule).
#
...
#   sbatch --export=ALL,RUN_DIR="$RUN_DIR",MANIFEST_PATH="$RUN_DIR/manifests/qwen3_ae_manifest.jsonl" \
#     slurm_scripts/submit_qwen_ae.sh
...
mkdir -p logs "$RUN_DIR/generation/qwen3" \
...
echo "=== Stage AE Generation — Qwen3 Array Task ==="
date
hostname
...
# Verify we are on L40S (abort if not — prevents CPU-offload pathology).
...
python -m poc_stage_ae.run_ae_generation \
    ...
    --condition "$CONDITION" \
    --output-dir "$RUN_DIR/generation/qwen3"
