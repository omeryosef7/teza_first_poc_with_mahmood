...
| C2 | Refusal restoration is **not** the route to attack removal | Llama + Qwen3 | same | 160/model | prompt, dose-matched **and causally** | Llama n=4 gaps **0.0000**; Qwen3 n=8 refusal arm **worse**; **R-35: one patch removes 69.2% of the refusal rise while ASR stays within margin of knockout-only** | arm-vs-arm 0.0417 | same | zero-refusal arms; below-band L5 patch | **R + CAUSAL** |
...
| 3 | Causal rescue by activation patching? | **RUN — PR-13 Outcome C on ASR (R-35).** Handing back clean demo-position activations at L14 recovers only **16.7%** of the ASR effect (within margin of knockout-only), but removes **69.2%** of the refusal rise (35→17 rows, >2× margin); the below-band L5 control moves refusal by **0.0000**. **The two effects have different substrates, shown causally.** |
