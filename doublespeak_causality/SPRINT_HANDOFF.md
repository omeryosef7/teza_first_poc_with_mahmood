...
- **RQ1 Necessity — CONFIRMED.** DS←Neutral patch: no effect early (L0–4), P(harm) → 0 from mid-layers. Controls: identity reproduces baseline (`id_max_dev` 0.008 potato ≪ effect); random norm-matched patch **126× (potato) / 8181× (mango)** weaker than the necessity drop. **[FLAGGED 2026-07-29 — RESULTS_FREEZE_AUDIT.md: this 126x/8181x ratio is NOT reproducible from `outputs/stage2_patching_*_20260727_001943/stage2_results.json` under any interpretation, and in fact points the opposite way. DO NOT CITE until recomputed; the superseding, artefact-backed control is `necessity - random = +0.181 [-0.021, 0.383]`, which crosses zero.]**
