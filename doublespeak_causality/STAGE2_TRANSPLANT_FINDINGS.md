...
- **Positive control (job 694706, RESOLVED — see B4):** additive `d_Direct` under forced_choice is ALSO weak (+0.029 p_concept; label-flip max 0.27 at α=4), NOT the +0.971 CAUSAL_CORE documented. This is **not a regression** — the current pipeline reproduces the on-disk CAUSAL_CORE artifact (693571: +0.028); the +0.971 doc claim is unbacked on disk (doc-vs-artifact drift, flagged). **This strengthens the finding:** on the reproducible pipeline, NO local codeword intervention — full-state transplant OR additive direction — installs the concept in a neutral context; only the surrounding context does. The validity control for "the machinery can produce concept reading" is therefore **DE_context itself (+0.35)** plus the exact self-transplant faithfulness — not d_Direct.
