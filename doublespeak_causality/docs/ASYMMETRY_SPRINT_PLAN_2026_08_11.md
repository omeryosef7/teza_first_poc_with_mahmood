...
> **⚠ THREE DESIGN CORRECTIONS to the paragraphs above, found by the §7.5 implementation audit (execution log 2026-08-12 16:05). These are corrections to this subsection as first drafted — the version above is NOT safe to run as written.**
>
> **(1) COMPUTE ASYMMETRY — the mechanism-vs-random contrast is compute-matched, but the headline per-prompt-vs-universal contrast is NOT.** A per-prompt arm at 200 steps *per prompt* spends **~37× the optimizer compute** of the universal arm (which spends 200 steps total to attack all 37). "Per-prompt beats universal" is therefore **not interpretable** as stated. **Required:** run a **compute-matched per-prompt arm at ~5 steps/prompt** (200 ÷ 37; the audit estimates this is essentially free, ~0.3 GPU-h) **in addition to** the full-budget arm, and **always report both budgets explicitly** alongside any per-prompt-vs-universal number. The full-budget arm is the threat model; the compute-matched arm is the fair contrast. Neither alone is sufficient.
>
> **(2) THE TWO ARMS ARE NOT EVALUATED ON THE SAME QUANTITY.** A per-prompt suffix is evaluated **on the very prompt it was optimized for** — there is **no transfer/generalization component at all**. The universal number (0.465/0.464) is a **transfer** result on held-out prompts. Comparing them directly is apples-to-oranges and would overstate per-prompt success. **Required:** state this explicitly wherever the numbers appear, and report the apples-to-apples reference — **the universal suffix's ASR on the SAME 37 prompts** the per-prompt arm attacked. *(This is not a leakage violation of §3.5: no hyperparameter, layer, threshold or seed is selected using test outcomes — the per-prompt attack simply is the threat model. But it is not a held-out number and must never be labelled one.)*
>
> **(3) THE COMPARISON MUST USE THE POSITION-CORRECTED UNIVERSAL ARMS.** At n=1, `legacy_fixed` reduces exactly to `per_task_suffix` (its index is `suffix_slice.stop - 1` computed from `train_tasks[0]`, which at n=1 *is* the prompt — verified at `gcg_optimizer.py:676-688`). So a per-prompt refusal arm compared against the **published legacy** universal arms would **partly measure the D1 bug fix rather than the per-prompt setting**. Compare against the **position-corrected** arms (`asym_p3_arm07p*` / `arm07pr*`), and pass **`--refusal-dir-position-mode per_task_decision`** on the per-prompt arms too — D1 vanishes at n=1 but **D2 (the within-task template offset) does not**.

**Two silent-failure modes this experiment must guard (both exit 0 while producing garbage).** Verified in the audit:
- **Empty run.** The optimizer keeps `split=="train"` rows, falling back to all rows only if that list is empty (`gcg_optimizer.py:617-619`). A 1-row manifest whose split is `test` therefore **requires `--split all`** at the call site, or `grad_accum` stays `None`, the loop breaks at step 0 (`:884-885`), and an **empty `ITERATION_LOG` is written with exit code 0**. Never rewrite the row's split label to dodge this — that corrupts provenance.
- **Cross-prompt resume.** `config_hash()` deliberately excludes `run_id`, `output_dir` **and `manifest_path`** (`config.py:192-224`). Two prompts sharing an `output_dir` would load **each other's `checkpoint.pt` with no hash mismatch** and silently optimize the wrong prompt. Every prompt needs its own run dir; assert uniqueness before launching.

Both are guarded by `scripts/split_manifest_perprompt.py` (emits the correct `--split` flag into the joblist and asserts unique output dirs). Post-run, also assert each `ITERATION_LOG` has the expected row count with `n_train_tasks == 1`.
