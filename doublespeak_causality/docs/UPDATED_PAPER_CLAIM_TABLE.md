...
|---|---|---|---|---|---|---|---|---|
| **A1** | The causal refusal direction is **unusually EASY** to reach from suffix-token perturbations — H1 (input-reachability failure) is rejected | `asym_p1_reachability.py`, `‖Jᵀv‖` at `decision`, hs[19] | train 40 / test 37 | **14.3× / 15.0×** vs 100 isotropic randoms (pct 1.000); **3.4×** vs other-layer refusal dirs (pct 0.983); 6.7×/7.1× vs the covariance-matched draws | mean percentile among controls **1.000 / 0.983** | 100 isotropic + 3 other-layer + 1 foreign. **The covariance-matched family is DEGENERATE (97–99 % mutually parallel) and is reported as "vs the single dominant residual direction", not as a 100-draw null** | **VERIFIED** (both splits; replicates under 4-bit NF4) | one base point (init suffix); cross-family smoke passed on Phi, full run pending |
| **A2** | The **first-order surrogate that discrete search relies on is invalid at one-token step size**, and *more so* for the refusal direction than for a typical activation-like direction | ε-scan, `asym_p1_reachability.py` | train 40 / test 37 | r = **0.84 / 0.81** at ε=0.10 → **−0.002 / −0.324** at ε=1.0; strict null retains **+0.204 / +0.334** | 120 probes per ε per split | all four direction families | **VERIFIED** (both splits) | bf16 gradients; one model |
