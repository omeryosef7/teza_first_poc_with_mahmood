...
| **A2** | The **first-order surrogate that discrete search relies on is invalid at one-token step size**, and *more so* for the refusal direction than for a typical activation-like direction | ε-scan, `asym_p1_reachability.py` | train 40 / test 37 | r = **0.84 / 0.81** at ε=0.10 → **−0.002 / −0.324** at ε=1.0; strict null retains **+0.204 / +0.334** | 120 probes per ε per split | all four direction families | **VERIFIED on Llama (both splits); CROSS-FAMILY PARTIAL NEGATIVE** | On Phi-4-mini the surrogate degrades (0.535→**+0.214**) but does **not** collapse, and the mechanism-vs-null ordering **inverts** (+0.214 vs +0.093). H2′ is a Llama result, not a general property. Phi's peak r is also lower (0.535 vs 0.840), so a noisier probe is a live alternative explanation |
