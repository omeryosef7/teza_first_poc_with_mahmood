...
|---|---|---|---|---|---|---|---|---|
| **A1** | The causal refusal direction is **unusually EASY** to reach from suffix-token perturbations — H1 (input-reachability failure) is rejected | `asym_p1_reachability.py`, `‖Jᵀv‖` at `decision`, hs[19] | train 40 / test 37 | **6.74× / 7.07×** the covariance-matched null; 14.3×/15.0× isotropic; 3.4× other-layer refusal | mean percentile among 100 controls **0.998 / 0.979** | 100 isotropic + 100 covariance-matched + 3 other-layer + 1 foreign | **VERIFIED** (both splits) | one base point (init suffix), one model |
