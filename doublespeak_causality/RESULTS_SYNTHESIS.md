...
> Scope & honesty note. Single model (Llama-3.1-8B-Instruct), seed data. The causal claims now hold across an **18-codeword panel** (virus concept): timing is perfectly consistent (Direct peak L0 / Doublespeak peak L31, n=18, 95% CI of the diff [31,31]); necessity removes ~98% and conditional sufficiency (suff(DS)>>suff(Direct), 95% CI [0.027,0.047]) hold across the ~7-15 codewords that hijack. Necessity also replicates across 3 concepts / 2 readouts. Absolute Patchscopes magnitudes are modest (≤0.3); hijacking strength is concept- and demo-dependent (16/18 codewords hijack for virus). Remaining external validity: **second model / more harmful concepts** (multi-model scaling not yet run).
