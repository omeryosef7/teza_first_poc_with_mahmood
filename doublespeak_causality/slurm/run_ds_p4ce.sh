#!/bin/bash
#SBATCH --job-name=ds_p4ce
#SBATCH --output=doublespeak_causality/logs/ds_p4ce_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_p4ce_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_ds_p4ce.sh
#
...
echo "=== run_ds_p4ce smoke ==="; date; hostname
GPU_TYPE="$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1 || true)"
case "$GPU_TYPE" in *L40S*|*l40s*) echo "GPU ok: $GPU_TYPE";; *) echo "ERROR need L40S got '$GPU_TYPE'"; exit 1;; esac
python -u doublespeak_causality/scripts/phase4c_carryedge.py --bench "${DSBENCH:-doublespeak_causality/data/bench/bench_clearharm_v2.json}" --carry "${DSCARRY:-L14H4_L14H5_L14H23_L15H8_L15H11_L17H24_L17H27_L18H20_L21H10}" --n-prompts "${DSNPROMPTS:-2}" --splits "${DSSPLITS:-dev,heldout}"
