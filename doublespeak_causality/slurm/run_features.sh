#!/bin/bash
#SBATCH --job-name=ds_features
#SBATCH --output=doublespeak_causality/logs/ds_features_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_features_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_features.sh
#
# Phase 5 feature extraction (21) — forward-only, fast. MODEL/SCREEN/MATRIX via --export.
#   sbatch --export=ALL,DSMODEL="meta-llama/Llama-3.1-8B-Instruct",DSSCREEN=doublespeak_causality/outputs/behavioral_screen_curated_v1,DSMATRIX=doublespeak_causality/data/behavioral_benchmark/screening_matrix_curated_v1.json run_features.sh
set -euo pipefail
