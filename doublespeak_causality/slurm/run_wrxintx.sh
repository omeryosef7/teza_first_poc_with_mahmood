#!/bin/bash
#SBATCH --job-name=ds_wrxintx
#SBATCH --output=doublespeak_causality/logs/ds_wrxintx_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_wrxintx_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_wrxintx.sh
...
python -u doublespeak_causality/scripts/phase_write_refusal_interaction.py \
  --bench "$DSBENCH" --model "$DSMODEL" --refusal-dir "$DSREFDIR" --write-layers "$DSWL" --n "$DSN" --splits "$DSSPLITS" --seed "$DSSEED"
echo "=== done ==="; date
