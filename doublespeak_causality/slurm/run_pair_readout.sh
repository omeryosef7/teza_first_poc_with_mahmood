#!/bin/bash
#SBATCH --job-name=ds_pairreadout
#SBATCH --output=doublespeak_causality/logs/ds_pairreadout_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_pairreadout_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_pair_readout.sh
#
# CAUSAL_CORE_PLAN §16.4 (S2 gate): validate the fixed-pair SAFE semantic readouts against
# the DIRECT_CONCEPT positive control and the NEUTRAL_CODEWORD negative control.
#   sbatch --export=ALL,DSBENCH=doublespeak_causality/data/pair_benchmark/pair_carrot_bomb.json run_pair_readout.sh
set -euo pipefail
