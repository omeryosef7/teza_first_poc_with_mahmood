#!/bin/bash
#SBATCH --job-name=ds_behrinj
#SBATCH --output=doublespeak_causality/logs/ds_behrinj_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_behrinj_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_behav_refinject.sh
...
python -u doublespeak_causality/scripts/phase_behav_refusal_inject.py \
  --bench "$DSBENCH" --model "$DSMODEL" --max-new "$DSMAXNEW" --refusal-pt "$DSREFPT" --alphas "$DSALPHAS" ${DSSAVEGEN:+--save-gen} ${DSINJLAYER:+--inject-layer $DSINJLAYER} --n "$DSN" --splits "$DSSPLITS" --seed "$DSSEED"
echo "=== done ==="; date
