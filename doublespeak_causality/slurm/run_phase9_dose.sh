...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_phase9_dose.sh
#
# CAUSAL_CIRCUIT_MASTER_PLAN Phase 9 dose-response (interpolated MLP patch)
...
: "${DSNPROMPTS:=0}"
: "${DSLAYERS:=9}"
: "${DSALPHAS:=0,0.25,0.5,0.75,1.0,1.5,2.0}"
...
echo "=== Phase9 dose: $DSBENCH n=$DSNPROMPTS layers=$DSLAYERS alphas=$DSALPHAS ==="; date; hostname; echo "git=$(git rev-parse HEAD 2>/dev/null||echo NA)"
GPU_TYPE="$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1 || true)"
case "$GPU_TYPE" in *L40S*|*l40s*) echo "GPU ok: $GPU_TYPE";; *) echo "ERROR need L40S got '$GPU_TYPE'"; exit 1;; esac
python -u doublespeak_causality/scripts/phase9_dose.py \
  --bench "$DSBENCH" --model "$DSMODEL" --splits dev,heldout --layers "$DSLAYERS" --alphas "$DSALPHAS" --n-prompts "$DSNPROMPTS"