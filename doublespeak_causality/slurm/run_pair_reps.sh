#!/bin/bash
#SBATCH --job-name=ds_pairreps
#SBATCH --output=doublespeak_causality/logs/ds_pairreps_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_pairreps_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_pair_reps.sh
#
# CAUSAL_CORE_PLAN §16.5 (S3): extract fixed-pair reps across all layers x positions x components
# for ONE readout template (the prompt suffix must be constant or the conditions stop being matched).
#   sbatch --export=ALL,DSREADOUT=cloze run_pair_reps.sh
set -euo pipefail
...
: "${DSSUB:=64}"
: "${DSLIMIT:=}"
for v in DSMODEL DSBENCH DSREADOUT DSSUB DSLIMIT; do
...
echo "=== pair rep extraction: $DSMODEL readout=$DSREADOUT ==="; date; hostname; echo "git=$(git rev-parse HEAD 2>/dev/null||echo NA)"
GPU_ALL="$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null || true)"; GPU_TYPE="${GPU_ALL%%$'\n'*}"
case "$GPU_TYPE" in *L40S*|*l40s*) echo "GPU ok: $GPU_TYPE";; *) echo "ERROR need L40S got '$GPU_TYPE'"; exit 1;; esac
python -u doublespeak_causality/32_extract_pair_reps.py \
  --bench "$DSBENCH" --model "$DSMODEL" --out-root doublespeak_causality/outputs \
  --readout "$DSREADOUT" --subsample "$DSSUB" $LIMIT_ARG
echo "=== done ==="; date
