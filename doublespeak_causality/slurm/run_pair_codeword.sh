#!/bin/bash
#SBATCH --job-name=ds_paircw
#SBATCH --output=doublespeak_causality/logs/ds_paircw_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_paircw_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_pair_codeword.sh
#
# CAUSAL_CORE_PLAN §8.1 (S13): codeword property study -- explicitly tests Matan's
# hypothesis that a codeword FARTHER from the concept in static-embedding space is a
# better carrier. Demonstration text is held identical across codewords.
#   sbatch --export=ALL,DSNDEMOS=12 run_pair_codeword.sh
set -euo pipefail
...
: "${DSREADOUT:=cloze}"
: "${DSSTYLES:=news:narrative:technical}"
: "${DSNDEMOS:=12}"
for v in DSMODEL DSBENCH DSREADOUT DSSTYLES DSNDEMOS; do
...
echo "=== pair codeword study: $DSMODEL readout=$DSREADOUT n_demos=$DSNDEMOS ==="
date; hostname; echo "git=$(git rev-parse HEAD 2>/dev/null||echo NA)"
GPU_ALL="$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null || true)"; GPU_TYPE="${GPU_ALL%%$'\n'*}"
case "$GPU_TYPE" in *L40S*|*l40s*) echo "GPU ok: $GPU_TYPE";; *) echo "ERROR need L40S got '$GPU_TYPE'"; exit 1;; esac
python -u doublespeak_causality/39_codeword_study.py \
  --bench "$DSBENCH" --model "$DSMODEL" --out-root doublespeak_causality/outputs \
  --readout "$DSREADOUT" --demo-styles "$STYLES" --n-demos "$DSNDEMOS"
echo "=== done ==="; date
