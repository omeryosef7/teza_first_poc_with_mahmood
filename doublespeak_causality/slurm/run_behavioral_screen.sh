#!/bin/bash
#SBATCH --job-name=ds_behscreen
#SBATCH --output=doublespeak_causality/logs/ds_behscreen_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_behscreen_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_behavioral_screen.sh
#
# Phase 2 behavioral screen: generate Direct/Neutral/Doublespeak on the benchmark
# matrix and label triplets + eligibility. MODEL/MATRIX/TAG via --export.
#   sbatch --export=ALL,DSMODEL="meta-llama/Llama-3.1-8B-Instruct",DSMATRIX=doublespeak_causality/data/behavioral_benchmark/screening_matrix_v1.json,DSTAG=llama8b run_behavioral_screen.sh
set -euo pipefail
...
: "${DSLIMIT:=}"          # cap #conditions (thinking-on is slow; first N conditions are deterministic)
LIM_ARG=""; [ -n "$DSLIMIT" ] && LIM_ARG="--limit $DSLIMIT"
...
python -u doublespeak_causality/17_validate_behavioral_triplets.py \
  ...
  --enable-thinking "$DSTHINK" --max-new-tokens "$DSMAXTOK" $LIM_ARG \
