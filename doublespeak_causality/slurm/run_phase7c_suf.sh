#!/bin/bash
#SBATCH --job-name=ds_suf
#SBATCH --output=doublespeak_causality/logs/ds_suf_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_suf_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_phase7c_suf.sh
#
# CAUSAL_CIRCUIT_MASTER_PLAN Phase 7c carry-head sufficiency install
...
echo "=== Phase7c suf: $DSBENCH n=$DSNPROMPTS carry=$DSCARRY ==="; date; hostname; echo "git=$(git rev-parse HEAD 2>/dev/null||echo NA)"
GPU_TYPE="$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1 || true)"
case "$GPU_TYPE" in *L40S*|*l40s*) echo "GPU ok: $GPU_TYPE";; *) echo "ERROR need L40S got '$GPU_TYPE'"; exit 1;; esac
python -u doublespeak_causality/scripts/phase7c_sufficiency.py \
