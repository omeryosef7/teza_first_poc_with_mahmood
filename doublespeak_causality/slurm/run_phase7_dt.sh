#!/bin/bash
#SBATCH --job-name=ds_p7dt
#SBATCH --output=doublespeak_causality/logs/ds_p7dt_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_p7dt_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_phase7_dt.sh
#
# CAUSAL_CIRCUIT_MASTER_PLAN Phase 7 DIRECT-vs-TOTAL path decomposition (candidate heads)
...
: "${DSNPROMPTS:=0}"
: "${DSHEADS:?set DSHEADS like L14H5-L15H8 or comma list}"
for v in DSMODEL DSBENCH DSNPROMPTS; do
  case "${!v}" in *,*) echo "ERROR: $v='${!v}' has a comma; --export truncates comma-lists."; exit 1;; esac
done
echo "=== Phase7 dt: $DSBENCH n=$DSNPROMPTS heads=$DSHEADS ==="; date; hostname; echo "git=$(git rev-parse HEAD 2>/dev/null||echo NA)"
GPU_TYPE="$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1 || true)"
case "$GPU_TYPE" in *L40S*|*l40s*) echo "GPU ok: $GPU_TYPE";; *) echo "ERROR need L40S got '$GPU_TYPE'"; exit 1;; esac
python -u doublespeak_causality/scripts/phase7_direct_total.py \
  --bench "$DSBENCH" --model "$DSMODEL" --splits dev,heldout --heads "$DSHEADS" --n-prompts "$DSNPROMPTS"