#!/bin/bash
#SBATCH --job-name=ds_p5bq
#SBATCH --output=doublespeak_causality/logs/ds_p5bq_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_p5bq_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_ds_p5bq.sh
#
...
echo "=== run_ds_p5bq smoke ==="; date; hostname
GPU_TYPE="$(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null | head -1 || true)"
case "$GPU_TYPE" in *L40S*|*l40s*) echo "GPU ok: $GPU_TYPE";; *) echo "ERROR need L40S got '$GPU_TYPE'"; exit 1;; esac
python -u doublespeak_causality/scripts/phase5b_qkv.py --bench "${DSBENCH:-doublespeak_causality/data/bench/bench_curated.json}" --splits "${DSSPLITS:-dev}" --carry "${DSCARRY:-L14H4_L15H8}" --n-prompts "${DSNPROMPTS:-2}"
