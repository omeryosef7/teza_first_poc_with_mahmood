#!/bin/bash
#SBATCH --job-name=ds_stage4lko_llama8b
#SBATCH --output=doublespeak_causality/logs/ds_stage4lko_%j.out
#SBATCH --error=doublespeak_causality/logs/ds_stage4lko_%j.err
#SBATCH --ntasks=1
...
#   sbatch --cpus-per-task=2 --mem=32G --time=00:40:00 --nodelist=n-802,n-803,n-804,n-805,t-806 slurm/run_stage4lko_llama8b.sh
#
...
MARK="doublespeak_causality/outputs/.stage4lko_llama8b.COMPLETE"
...
python -u doublespeak_causality/10_layerwise_knockout.py \
    --model "$MODEL" --data doublespeak_causality/data/seed_concepts_gpt4omini.json --templated --readout-layer 30 --only virus_muffin
