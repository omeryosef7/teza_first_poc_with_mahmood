...
CLAIMS = [
    ...
    ,
    # ============ Behavioral-causality sprint (2026-08-07/08): refusal-suppression decision state ============
    dict(id="RS-01", phase="GATE-A", status="VERIFIED",
         claim="Gate A (§3): the DS-suppressed refusal representation is causally RESTORABLE by a residual overwrite at the decision token, L15-18. Patching the DS resid_pre with the matched Direct donor restores the validated refusal projection to frac~0.93 of the direct-vs-ds gap at L18 (ratio-of-means: train 0.936, dev 0.931, test 0.926), Holm-p~0, replicated on the frozen test split; attn_out/mlp_out barely restore (frac ~0.06/0.33), so the suppression is carried in the accumulated residual, not a single sub-block write. Locality no-op: self-swap max|restore| <= 5e-6.",
         source="P_REFUSAL_SUPPRESSION_LOCALIZATION.md §3 (clearharm; NULL on generated)",
         dirs=["outputs/refsuploc_clearharm_20260807_163312_732161"],
         script="scripts/phase_refusal_suppression_localize.py", recompute="python scripts/analyze_refsuploc.py outputs/refsuploc_clearharm_20260807_163312_732161",
         note="Headline 'frac (ratio-of-means)' in the report = restore_mean / direct_minus_ds_gap_mean (test 1.4422/1.5583 = 0.926, matches the table exactly). The summary also carries a mean-of-per-item-ratios 'frac_mean' (test 0.943) which differs slightly by construction; both are >~0.93. Representational endpoint only -- behavioral confirmation is GB-01/GB-02. NULL on the generated cohort (DS net-negative there), reported per-cohort.",
         checks=[dict(kind="summary", dir="outputs/refsuploc_clearharm_20260807_163312_732161", path="selfswap_max_abs_restore", expect=5e-06, tol=1e-7),
                 dict(kind="summary", dir="outputs/refsuploc_clearharm_20260807_163312_732161", path="by_split.test.cells.resid_pre|direct|L18.restore_ci.0", expect=1.4422),
                 dict(kind="summary", dir="outputs/refsuploc_clearharm_20260807_163312_732161", path="by_split.test.direct_minus_ds_gap_mean", expect=1.5583),
                 dict(kind="summary", dir="outputs/refsuploc_clearharm_20260807_163312_732161", path="by_split.train.cells.resid_pre|direct|L18.frac_mean", expect=0.9837),
                 dict(kind="summary", dir="outputs/refsuploc_clearharm_20260807_163312_732161", path="by_split.test.cells.attn_out|direct|L18.frac_mean", expect=0.059),
                 dict(kind="summary", dir="outputs/refsuploc_clearharm_20260807_163312_732161", path="by_split.test.cells.mlp_out|direct|L18.frac_mean", expect=0.3289)]),
    dict(id="GB-01", phase="GATE-B", status="VERIFIED", effect=-0.1412,
         claim="Gate B forward (§23): restoring the Direct decision-token residual at L17 DURING GENERATION lowers ASR to ~direct_base level -- ΔASR -0.141 on clearharm train (McNemar p=0.012, discordant b=16/c=4), -0.186 on dev (p=0.008). Specificity: the norm-matched random donor moves ASR the OPPOSITE way (+0.141). Locality: self-swap ΔASR ~0 (p=1.0). This converts §3's representational Gate A into a behavioral-causality claim (plan claim D).",
         source="P_GATE_B_DECISION_STATE_BEHAVIORAL.md §23 (clearharm; NULL on generated)",
         dirs=["outputs/refdecpatch_clearharm_20260807_194210_732388"],
         script="scripts/phase_refusal_decision_patch_behav.py", recompute="python scripts/validate_all_outputs.py outputs/refdecpatch_clearharm_20260807_194210_732388",
         note="Effects are 7-10x the ~2pp within-run judge floor (NF-01) and have asymmetric McNemar discordance symmetric noise cannot produce. Test cell underpowered (ds_base ASR 0.167). Generated cohort DONE=NULL (732389, DS net-negative there) -- expected and coherent.",
         checks=[dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_194210_732388", path="by_split.train.vs_ds_base.ds_dpatch_direct_L17.delta_ASR", expect=-0.1412),
                 dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_194210_732388", path="by_split.train.vs_ds_base.ds_dpatch_direct_L17.mcnemar_p", expect=0.01182),
                 dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_194210_732388", path="by_split.dev.vs_ds_base.ds_dpatch_direct_L17.mcnemar_p", expect=0.00781),
                 dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_194210_732388", path="by_split.train.vs_ds_base.ds_dpatch_self_L17.delta_ASR", expect=0.0118),
                 dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_194210_732388", path="by_split.train.vs_ds_base.ds_dpatch_rand_L17.delta_ASR", expect=0.1412)]),
    dict(id="GB-02", phase="GATE-B", status="VERIFIED", null_by_design=True, effect=0.0471,
         claim="Bidirectional §23: inserting the DS decision-token residual into a REFUSING Direct prompt does NOT induce compliance -- reverse ΔASR +0.047 at L17 (p=0.34, ~self no-op +0.012). So the DS decision state is not SUFFICIENT for compliance; Gate B stays PASS(forward), not STRONG. Forward reproduces in the same run (ΔASR -0.153, p=0.011). A norm-matched RANDOM perturbation raises ASR in both directions (reverse +0.341, p<1e-3) -- decision-state fragility, not a specific DS compliance vector.",
         source="P_GATE_B_DECISION_STATE_BEHAVIORAL.md §23 bidirectional (732560)",
         dirs=["outputs/refdecpatch_clearharm_20260807_221715_732560"],
         script="scripts/phase_refusal_decision_patch_behav.py", recompute="python scripts/validate_all_outputs.py outputs/refdecpatch_clearharm_20260807_221715_732560",
         note="Well-controlled negative: structured DS/self donors inert, only random moves ASR (and it does so symmetrically, an OOD-magnitude effect). Mechanism reading: an intact refusal decision state is REQUIRED to refuse; there is no DS signal that overrides it.",
         checks=[dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_221715_732560", path="by_split.train.vs_direct_base.direct_dpatch_ds_L17.delta_ASR", expect=0.0471),
                 dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_221715_732560", path="by_split.train.vs_direct_base.direct_dpatch_ds_L17.mcnemar_p", expect=0.34375),
                 dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_221715_732560", path="by_split.train.vs_direct_base.direct_dpatch_self_L17.delta_ASR", expect=0.0118),
                 dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_221715_732560", path="by_split.train.vs_direct_base.direct_dpatch_rand_L17.delta_ASR", expect=0.3412),
                 dict(kind="summary", dir="outputs/refdecpatch_clearharm_20260807_221715_732560", path="by_split.train.vs_ds_base.ds_dpatch_direct_L17.delta_ASR", expect=-0.153)]),
    dict(id="DEF-01", phase="GATE-F", status="VERIFIED", effect=-0.2235,
         claim="Defense non-selectivity (§19-21/§19.3): calibrated refusal-restoration DEFENDS the attack (L18 ΔASR -0.224, p=2e-5; L16 -0.153; refusal-axis-specific -- random control does not defend) but OVER-REFUSES benign at every dose (L18 benign over-refusal +0.376; the dose sweep 0.25->1.0 raises both curves together, attack/over-refusal ratio ~constant, no selective dose) AND under intent-gating the gate fires on benign >= attack (benign fire-rate 0.871 vs ds 0.812), so gated == unconditional. => Gate F FAIL: no selective mechanism-derived defense exists.",
         source="P_DEFENSE_UTILITY.md §19-21 (732688 fixed-dose, 732750 dose sweep) + §19.3 (732795 gated)",
         dirs=["outputs/defense_util_clearharm_20260808_010203_732688", "outputs/defense_util_clearharm_20260808_025303_732750", "outputs/defense_gated_clearharm_20260808_053211_732795"],
         script="scripts/phase_defense_utility.py + scripts/phase_defense_gated.py", recompute="python scripts/validate_all_outputs.py outputs/defense_util_clearharm_20260808_010203_732688 outputs/defense_gated_clearharm_20260808_053211_732795",
         note="Deep result: the intent circuit is behaviorally epiphenomenal while the refusal circuit drives behavior but is not intent-selective (DS attack suppresses refusal to a level indistinguishable from a benign attack-structured request), so neither circuit alone yields a selective defense. validate_all_outputs mis-detects these dirs as 'behav' schema (a validator coverage gap, §36, not a data issue); the numeric checks below re-derive the headline figures straight from summary.json.",
         checks=[dict(kind="summary", dir="outputs/defense_util_clearharm_20260808_010203_732688", path="by_split.train.by_layer.18.delta_ASR", expect=-0.2235),
                 dict(kind="summary", dir="outputs/defense_util_clearharm_20260808_010203_732688", path="by_split.train.by_layer.18.benign_over_refusal", expect=0.3764),
                 dict(kind="summary", dir="outputs/defense_util_clearharm_20260808_010203_732688", path="by_split.train.by_layer.16.delta_ASR", expect=-0.1529),
                 dict(kind="summary", dir="outputs/defense_util_clearharm_20260808_025303_732750", path=["by_split", "train", "by_layer", "L18_d0.75", "benign_over_refusal"], expect=0.2823),
                 dict(kind="summary", dir="outputs/defense_gated_clearharm_20260808_053211_732795", path="by_split.train.benign_fire_rate", expect=0.8706),
                 dict(kind="summary", dir="outputs/defense_gated_clearharm_20260808_053211_732795", path="by_split.train.ds_fire_rate", expect=0.8118),
                 dict(kind="summary", dir="outputs/defense_gated_clearharm_20260808_053211_732795", path="by_split.train.benign.gated_over_refusal", expect=0.3764)]),
    dict(id="NF-01", phase="NOISE-FLOOR", status="VERIFIED",
         claim="Noise floor (§1.2): greedy generation is deterministic (gen_determinism_rate = 1.000 in every cell -- no unexplained generation variance) and re-judging byte-identical text flips the StrongREJECT label on mean ~1-2% of items within-run (direct 0.008, DS 0.016, benign 0.020; any-flip up to ~6%). The BETWEEN-run judge floor (cross-run item-by-item on the DS baseline) is ~6pp. => any within-run |ΔASR| below ~2pp is uninterpretable; cross-run comparisons need ~6pp.",
         source="P1_2_BASELINE_DRIFT_ENVELOPE.md §1.2",
         dirs=["outputs/baseline_drift_clearharm_20260807_202212_732432"],
         script="scripts/phase_baseline_drift.py", recompute="python scripts/validate_all_outputs.py outputs/baseline_drift_clearharm_20260807_202212_732432",
         note="The ~6pp between-run floor is a cross-run derivation (drift 732432 majority-of-3 DS ASR 0.306 vs Gate B 732388 single-judge 0.271 = 3.5pp; 5/85 items disagree = 5.9%) and is not a single summary.json leaf, so only the within-run within-summary figures are machine-checked below. This backs Gate B: its paired within-run effects (7-10x the 2pp floor, asymmetric discordance) survive.",
         checks=[dict(kind="summary", dir="outputs/baseline_drift_clearharm_20260807_202212_732432", path="by_split.train.conditions.doublespeak.gen_determinism_rate", expect=1.0),
                 dict(kind="summary", dir="outputs/baseline_drift_clearharm_20260807_202212_732432", path="by_split.train.conditions.direct.gen_determinism_rate", expect=1.0),
                 dict(kind="summary", dir="outputs/baseline_drift_clearharm_20260807_202212_732432", path="by_split.train.conditions.direct.judge_flip_rate_mean", expect=0.0078),
                 dict(kind="summary", dir="outputs/baseline_drift_clearharm_20260807_202212_732432", path="by_split.train.conditions.doublespeak.judge_flip_rate_mean", expect=0.0157),
                 dict(kind="summary", dir="outputs/baseline_drift_clearharm_20260807_202212_732432", path="by_split.train.conditions.benign.judge_any_flip_rate", expect=0.0588)])
