...
def test_register_function():
    ...
    class Function(BaseModel):
        model_config = {
            "title": f"Input schema for `{test_function.__name__}`"
        }
        ...
        h: DummyModel = Field(
            description="Description of h", default=DummyModel(a=1)
        )
    ...
    assert (
        runtime.functions["test_function"].parameters.model_json_schema()
        == Function.model_json_schema()
    )
    assert runtime.functions["test_function"].return_type == types.NoneType
    assert (
        runtime.functions["test_function"].full_docstring
...
def test_register_function_return_union_type():
    ...
    assert (
        runtime.functions["test_function"].return_type
        == int | str | dict[str, list[int]]
    )
    assert (
        runtime.functions["test_function"].full_docstring
...
def test_run_stateful_function():
    ...
    env = EmptyEnv()
    result_1, error_1 = runtime.run_function(
        env, "test_stateful_sum", {"a": 1, "b": 2}
    )
    assert result_1 == 3
    assert env.list_state.l == [3]
    assert error_1 is None

    result_2, error_2 = runtime.run_function(
        env, "test_stateful_sum", {"a": 4, "b": 5}
    )
    assert result_2 == 9
...
def test_run_nested_function_call():
    ...
    result, error = runtime.run_function(
        ...
        {
            "a": 1,
            "b": FunctionCall(function="test_sum", args={"a": 1, "b": 2}),
        },
    )
...
def test_run_error_function():
    ...
    result, error = runtime.run_function(
        ...
        "test_sum_2",
        {
            "a": 1,
            "b": FunctionCall(
                function="test_sum_2", args={"a": 1, "b": "abc"}
            ),
        },
    )
    assert result == ""
