...
def aggregate_results(results: list[dict[tuple[str, str], bool]]) -> float:
    all_results = [
        result for results in results for result in results.values()
    ]
...
def benchmark_suite_with_injections(
    ...
    if injection_tasks is not None:
        injection_tasks_to_run = {
            injection_task_id: suite.get_injection_task_by_id(
                injection_task_id
            )
    ...
    if not attack.is_dos_attack:
        for (
            injection_task_id,
            injection_task,
        ) in injection_tasks_to_run.items():
            successful, _ = run_task_without_injection_tasks(
                suite, agent_pipeline, injection_task, logdir, force_rerun
            )
...
def run_task_without_injection_tasks(
    ...
    if task_result is not None and not force_rerun:
        logging.info(
            f"Skipping task '{task.ID}' with no injections because it was already run."
        )
...
def benchmark_suite_without_injections(
    ...
    for user_task in user_tasks_to_run:
        utility, security = run_task_without_injection_tasks(
            suite, agent_pipeline, user_task, logdir, force_rerun
        )
...
class TaskResults(BaseModel):
    ...
    def check_messages(self) -> Self:
        if (
            len(self.messages) < 2
            and "claude-3-opus" not in self.pipeline_name
        ):
...
def load_suite_results(
    ...
    dict[tuple[str, str], TaskResults], dict[tuple[str, str], TaskResults]
]:
