...
class OutputLogger(Logger):
    def log_error(self, message: str):
        logging.warning(
            f"[red]API error: {message}[/red]", extra={"markup": True}
        )
    ...
    def __init__(self, logdir: str | None, live: Live | None = None):
        ...
        self.live = live
        self.table = Table(
            "Role", "Content", title="Chat log", show_lines=True
        )
    ...
    def log(self, messages: list[ChatMessage], **kwargs):
        ...
        injection_task_id = kwargs.get(
            "injection_task_id" or "unknown injection_task_id"
        )
        ...
        for message in messages:
            ...
            if role == "user":
                ...
                if "tool_call" in message:
                    role = (
                        f"{role} [bold]{message['tool_call'].function}[/bold]"
                    )
                role = tool(role)
            else:
                role = system(role)
            if self.live is None:
                logging_content = textwrap.shorten(
                    f"{content}{tool_calls_content}",
                    width=100,
                    placeholder="[...]",
                )
                if injection_task_id is not None:
                    current_task_id = (
                        rf"\[{user_task_id}]\[{injection_task_id}]"
                    )
                else:
...
class TraceLogger(Logger):
                ...
                json.dumps(
                    {
                        ...
                        "attack_type": (
                            attack_type if attack_type != "none" else None
                        ),
    ...
    def log(self, messages: list[ChatMessage]):
        user_task_id = self.context.get("user_task_id", "unknown_user_task_id")
        injection_task_id = self.context.get(
            "injection_task_id", "unknown_user_task_id"
        )
        suite_name = self.context.get("suite_name", "unknown_suite_name")
        pipeline_name = self.context.get(
            "pipeline_name", "pipeline_suite_name"
        )
        self.delegate.log(
            messages,
            user_task_id=user_task_id,
