...
class BaseAttack(abc.ABC):
    ...
    def __init__(
        self, task_suite: TaskSuite, target_pipeline: BasePipelineElement
    ) -> None:
    ...
    def get_injection_candidates(self, user_task: BaseUserTask) -> list[str]:
        ...
        _, _, _, responses, _ = gt_pipeline.query(
    ...
    def generate_injections(self) -> Injections:
        ...
        for user_task_id, user_task in track(
            self.task_suite.user_tasks.items()
        ):
            for (
                inj_task_id,
                inj_task,
            ) in self.task_suite.injection_tasks.items():
