...
class Inbox(BaseModel):
    ...
    def _create_emails(self) -> Self:
        ...
        if len(all_ids) != len(set(all_ids)):
            ...
            raise ValueError(
                f"Email IDs must be unique. Duplicates: {duplicate_ids}"
            )
        ...
        for email in self.initial_emails:
            ...
            for recipient in email_contacts:
                if recipient in existing_contacts:
                    continue
                if recipient not in [
                    contact.email for contact in self.contact_list
                ]:
    ...
    def _get_emails_by_status(self, status: EmailStatus) -> list[Email]:
        return [
            email for email in self.emails.values() if email.status == status
        ]
    ...
    def search_emails(
        self, query: str, sender: EmailStr | None = None
    ) -> list[Email]:
    ...
    def get_unread(self) -> list[Email]:
        unread_emails = [
            email for email in self.emails.values() if not email.read
        ]
...
def send_email(
        ...
        for attachment in attachments:
            ...
            if attachment["type"] == "file" or "file_id" in attachment:
                if "file_id" not in attachment:
                    raise ValueError(
                        "Attachment of type 'file' must have a 'file_id' field."
                    )
                parsed_attachments.append(
                    CloudDriveFileID(attachment["file_id"])
                )
            else:
                if "event_details" not in attachment:
                    raise ValueError(
                        "Attachment of type 'event' must have an 'event_details' field."
                    )
                parsed_attachments.append(
    ...
    return inbox.send_email(
        recipients, subject, body, parsed_attachments, cc, bcc
    )
...
def delete_email(
    inbox: Annotated[Inbox, Depends("inbox")], email_id: str
) -> str:
...
def search_contacts_by_name(
    inbox: Annotated[Inbox, Depends("inbox")], query: str
) -> list[EmailContact]:
