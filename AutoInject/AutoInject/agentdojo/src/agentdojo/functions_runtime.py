...
class FunctionsRuntime:
    ...
    def register_function(
        self, function: Callable[P, T] | Function
        ...
        try:
            ...
            raise Exception(
                f"Error when registering function {function.__name__} in {function.__module__}."
            ) from e
    ...
    def run_function(
        ...
        if function not in self.functions:
            if raise_on_error:
                raise ToolNotFoundError(
                    f"The requested function `{function}` is not available."
                )
            return (
                "",
                f"ToolNotFoundError: The requested function `{function}` is not available.",
            )
        ...
        if env is None and len(f.dependencies) > 0:
            raise ValueError(
                f"Function {function} has dependencies but no environment was provided."
            )
        else:
            env_args = {
