    ...
    match message["role"]:
        ...
        case "tool":
            if message["tool_call_id"] is None:
                raise ValueError(
                    "`tool_call_id` should be specified for OpenAI."
                )
...
class OpenAILLM(BasePipelineElement):
    ...
    def __init__(
        ...
        self.reasoning_effort: ChatCompletionReasoningEffort | None = (
            reasoning_effort
        )
    ...
    def query(
        ...
        openai_messages = [
            _message_to_openai(message, self.model) for message in messages
        ]
        openai_tools = [
...
class OpenAILLMToolFilter(BasePipelineElement):
    ...
    def query(
        ...
        messages = [
            *messages,
            ChatUserMessage(role="user", content=self.prompt),
        ]
        openai_messages = [
            _message_to_openai(message, self.model) for message in messages
        ]
        ...
        for tool_name, tool in runtime.functions.items():
            if (
                ...
                and tool_name in output["content"]
            ):
