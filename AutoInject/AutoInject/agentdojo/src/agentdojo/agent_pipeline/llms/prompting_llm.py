...
class BasePromptingLLM(OpenAILLM):
    ...
    def query(
        ...
        openai_messages = [
            _message_to_openai(message, self.model)
        ...
        for _ in range(self._MAX_ATTEMPTS):
            try:
                ...
                error_message = ChatUserMessage(
                    role="user",
                    content=f"Invalid function calling output: {e!s}",
                )
                completion = chat_completion_request(
                    self.client,
                    self.model,
                    [
                        *openai_messages,
                        _message_to_openai(error_message, self.model),
                    ],
                    [],
                    None,
                    self.temperature,
    ...
    def _parse_model_output(
        ...
        try:
            ...
            logger.warning(
                ...
                f"Original: {repr(message.content)}. Error: {e}"
            )
            ...
            if (fixed_content.startswith("[") or fixed_content.startswith("{")) and "(" not in fixed_content:
                try:
                    ...
                    if isinstance(json_data, list) and len(json_data) > 0 and isinstance(json_data[0], dict):
                        ...
                        function_thoughts_match = re.search(r"<function-thoughts>(.*?)</function-thoughts>", content_str, re.DOTALL)
                        if function_thoughts_match:
                            thoughts = function_thoughts_match.group(1).lower()
                            ...
                            function_patterns = [
                                r"`(\w+_?\w*)`",  # `function_name`
                                r"(\w+_?\w*)\s+function",  # function_name function
                                r"use\s+(?:the\s+)?(\w+_?\w*)",  # use the function_name
                                r"call\s+(\w+_?\w*)",  # call function_name
                            ]
                            inferred_function = None
                            for pattern in function_patterns:
                                match = re.search(pattern, thoughts)
                                if match:
                                    inferred_function = match.group(1)
                                    break
