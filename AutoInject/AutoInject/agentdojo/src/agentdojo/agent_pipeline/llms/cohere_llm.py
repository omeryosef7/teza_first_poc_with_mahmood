...
try:
    ...
    warnings.warn(
        DeprecationWarning(
            "You are using an old version of the Cohere SDK. Please upgrade to the latest version."
        )
    )
...
def _message_to_cohere(message: ChatMessage) -> CohereMessage:
    match message["role"]:
        ...
        case _:
            raise ValueError(
                f"Invalid message role for Cohere: {message['role']}"
            )


def _merge_tool_result_messages(
    messages: Sequence[CohereMessage],
) -> Sequence[CohereMessage]:
    messages = list(messages)
    i = 0
    while i < len(messages) - 1:
        message_1 = messages[i]
        message_2 = messages[i + 1]
        if isinstance(message_1, ToolMessage) and isinstance(
            message_2, ToolMessage
        ):
...
def _conversation_to_cohere(
    ...
    if len(messages) == 0:
        raise ValueError(
            "There should be at least one message in the conversation"
        )
            ...
            for all_of_definition in param_definition["allOf"]:
                if "$ref" in all_of_definition:
                    if "$defs" not in parameters:
                        raise ValueError(
                            f"Can't resolve $ref {all_of_definition['$ref']}"
                        )
                    enum = all_of_definition["$ref"].split("/")[-1]
                    param_type = JSON_TO_PYTHON_TYPES[
                        parameters["$defs"][enum]["type"]
                    ]
                    param_definition[
                        "description"
                    ] += f" (Possible values: {parameters['$defs'][enum]['enum']})"
                    break
            if not param_type:
                raise ValueError(
