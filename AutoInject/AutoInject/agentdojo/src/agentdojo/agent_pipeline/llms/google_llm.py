    ...
    if "properties" in openapi_parameters:
        for _, prop_value in openapi_parameters["properties"].items():
            if "anyOf" in prop_value:
                ...
                non_null_types = [
                    schema
                    for schema in prop_value["anyOf"]
    ...
    for message in messages:
        if any(part.function_response for part in message.parts):
            ...
            current_tool_message = genai.Content(
                parts=current_tool_message.parts
                + [part for part in message.parts if part.function_response]
            )
        else:
...
def _google_to_tool_call(function_call: genai.FunctionCall) -> FunctionCall:
    if function_call.name == "":
        function = (
            EMPTY_FUNCTION_NAME  # sometimes the API returns an empty string
        )
    else:
...
class GoogleLLM(BasePipelineElement):
    ...
    def query(
        ...
        google_messages = [
            _message_to_google(message) for message in other_messages
        ]
        google_messages = _merge_tool_result_messages(google_messages)
        google_functions = [
            _function_to_google(tool) for tool in runtime.functions.values()
        ]
        google_tools = (
        ...
        model = genai.GenerativeModel(
            ...
            system_instruction=(
                genai.Part.from_text(text=system_instruction)
