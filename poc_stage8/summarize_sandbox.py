...
GROUP_FIELDS = ("action_type", "reward_mode")
SUMMARY_FIELDS = (
    "count",
    "mean_reward",
    "reward_std",
    ...
    "reward_min",
    "reward_max",
    "used_stage7_row_rate",
)
CSV_FIELDS = (*GROUP_FIELDS, *SUMMARY_FIELDS)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Aggregate Stage 8 sandbox JSONL rows into compact summary files.")
...
def _aggregate_group(key: tuple[Any, ...], rows: list[dict[str, Any]]) -> dict[str, Any]:
    rewards = [value for row in rows if (value := _coerce_float(row.get("reward"))) is not None]
    used_stage7_values = [value for row in rows if (value := _coerce_bool(row.get("used_stage7_row"))) is not None]
    return {
        **_display_group(key),
        "count": len(rows),
        ...
        "used_stage7_row_rate": _mean([1.0 if value else 0.0 for value in used_stage7_values]),
    }
...
def run_summary(config: SummarizeConfig) -> dict[str, Any]:
    ...
    payload = {
        "stage": 8,
        "summary_type": "sandbox_reward_aggregate",
