...
OUT = "GCG_Summary_for_Mahmood_2026-07-19.pptx"
...
def title_slide():
    ...
    r3.text = ("Models: Qwen3-14B, Gemma4-E4B-it, DeepSeek-R1-Distill-Qwen-7B\n"
               ...
               "(now includes the v2 fixed-optimizer re-run: slides 10A–10D)")
...
add_body(s, [
    ...
    "**Verified real, not a scoring glitch:** all **66/66** v2 failures are genuine refusals; re-scoring old v1 rows with today's judge reproduced them exactly (**0/6 drift**).",
...
add_body(s, [
    ...
    "**Seed 45: 9.6%** vs 4.1% (**+5.5pp**).   **Seed 43: 7%** vs 3% (marginal).",
...
add_body(s, [
    ...
    "**9A (refusal-direction, λ=0.3): ~12.7%** vs ~2.3% baseline → **+~10pp uplift.**",
    "**9B (seed-45 CoT): ~8.2%** vs ~2.9% baseline → **+~5pp uplift.**",
    "**What collapses:** 5A / 7A seed-42 (~3% ≈ 2% baseline), and **Gemma4-10A ~1.4%** — *below* its 2.3% baseline (no uplift).",
    ...
    ("Caveat: 520-scale evals land partial (~800–2000 of 6240 rows) — these magnitudes will still move as the runs finish.", 1),
], size=15, top=1.9)
...
add_body(s, [
    ...
    "**Pilot running now:** 12 behaviours × 4 target styles × 3 seeds = **144 optimizations**; early-scored behaviours ~⅔ crackable, mean best ~17%, winners from checkpoints. Small samples (10 generation seeds) → wide error bars.",
...
NOTES = [
    ...
    [
        ...
        "IT'S REAL, NOT A SCORING BUG (pre-empt the obvious challenge): all 66/66 v2 failures are genuine refusals (no harmful-but-scored-zero); the 3 successes are genuinely harmful; and re-scoring 6 old v1 rows with today's judge reproduced them exactly (0/6 drift). Same eval code and judge for v1 and v2 — only the suffix differs.",
    ...
    [
        ...
        "Seed 45: 9.6% (+5.5pp). Seed 43: 7% (marginal).",
    ...
    [
        ...
        "SURVIVORS: 9A (refusal-direction, lambda=0.3) ~12.7% vs ~2.3% baseline = +~10pp. 9B (seed-45 CoT) ~8.2% vs ~2.9% = +~5pp. These KEEP real uplift under correct placement.",
    ...
    [
        ...
        "PILOT (running now): 12 behaviours x 4 target styles x 3 seeds = 144 optimizations, 2 shards, resume-safe. Early: ~2/3 of scored behaviours crackable, mean best ~17%, winners from checkpoints.",
