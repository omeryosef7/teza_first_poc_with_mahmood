...
PRODUCER UNDER TEST
-------------------
  scripts/dcs_bombness_specificity.py
  output  = outputs/boombness/dcs_analysis/dcs_bombness_specificity.json

  C-070 (M-12): this header used to pin `commit 1483f9c1, sha256 50e2dde6...`. That pin went STALE
  the moment the file changed for reasons unrelated to the numbers (the EXCLUDE_GLOBAL_RELABELS
  gate of C-061, the print-only progress ticks), and a stale pin in a verifier header is worse than
  no pin: a reader takes it for a checked fact. It is removed rather than refreshed, because
  nothing in this file ever read it. Bank binding, which IS checked, is done per run against
  `bank_file_sha16`.
...
def _stage_a(cfg, rep):
    ...
    # C-070 (M-12): C3 compared every run's signature to a reference run's and never asked whether
    # the fields EXIST. A runs-root with model/dtype/seed/tokenizer stripped from all 8
    # metadata.json files agrees perfectly on `None` and PASSED. Absent identity is not identity.
    IDENTITY_MUST_EXIST = ("layers", "model", "dtype", "position", "seed")
    for k, sg in sorted(sigs.items()):
        missing = [f for f in IDENTITY_MUST_EXIST if sg.get(f) in (None, (), [], "")]
        if missing:
            mismatch.append(f"{k}: identity fields ABSENT, not merely equal: {missing}")
...
def _stage_b(cfg, rep, ctx):
    ...
    # C-070 (H-4): this check SKIPS any node lacking a `picks` table and then reported success
    # over whatever survived -- so deleting every `picks` table produced
    # "[PASS] C5_FOLD_DISCIPLINE ... no instrument present". A check that passes when it bound to
    # nothing is dead by omission, which is the C-055/C-056 failure class this file exists to catch.
    if not fold_bad and not fold_ok:
        fold_bad.append("C5 bound to ZERO instruments: no node in the producer JSON carries a "
                        "`picks` table, so fold discipline was never actually checked. A check "
                        "with no bindings FAILS; it does not pass vacuously.")
    rep.add("C5_FOLD_DISCIPLINE", not fold_bad,
            ("every reported fold's n_train/n_test re-derives from the banks as strict "
             f"leave-one-group-out: {', '.join(fold_ok)}"
    ...
    # C-070 (H-4): same defect -- deleting every `n_domains` field left `ind_seen` empty and the
    # check printed "[PASS] ... 0 reported group-level statistics all fold over 6 domains".
    if not ind_bad and not ind_seen:
        ind_bad.append("C7 bound to ZERO statistics: no node in the producer JSON carries "
                       "`n_domains`, so the independence unit was never actually checked. A check "
                       "with no bindings FAILS; it does not pass vacuously.")
...
4b. C-070 (H-4): DEAD-BY-OMISSION was a real defect here and is now closed for C5 and C7 -- both
   used to SKIP nodes lacking the field they check and then report success over the survivors, so
   deleting every `picks` table or every `n_domains` field produced a PASS. Both now FAIL when they
   bind to nothing. ⛔ THE SAME PATTERN MAY EXIST IN OTHER CHECKS: an adversarial review found that
   15 hand-written mutations against the real producer JSON left 10 UNCAUGHT, including a
   self-consistent fabricated `P2_primary.per_domain := 0.95` (with mean_acc and n_above_chance
   updated to match), which verifies at rc=0 with all 14 checks PASS. That specific hole is
   limitation 2 below, by design; `dcs_verify_pr035_primary.py` exists to close it.
