...
python -u src/boombness/judge_boombness.py --gens outputs/boombness/score_behavior/ab_base_20260818_185458_3888976 --bank "$BANK" --tag a24d_base
i=0
while IFS=: read -r tag gens; do
  [ -n "$tag" ] || continue
  python -u src/boombness/judge_boombness.py --gens "$gens" --bank "$BANK" --tag "a24d_${tag}" &
  i=$((i+1)); if [ $((i % 6)) -eq 0 ]; then wait; fi
done < "$R/outputs/boombness/argsfiles/a24d_runs.txt"
wait
